 ...
 * 021 02/09/99 EAB Changed OLD_TILT to NEW_TILT to follow convention of 
 *		    enabling new functions
 * 022 /02/10/99 Opps wanted new_tilt disabled for submital until it's checked out
 * 023 02/12/99	EAB Added comments and chnaged one ifdef
 * 024 02/22/99 Modified to support new ad_buid and multiple vdef files
				and turned off ad for defalt
   025 02/23/99 EAB spanish should include latin as well for the vdef file
   026 03/19/99 EAB Fixed for NWSNOAA plus fixed bug in spanish an german 
				vdef fetch also nwsnoaa needs to fetch yet another vdef file
   027 03/19/99 EAB Added chages to support the 16 build for Noaa SPanish inluding
				a new nwsnoaa tuning file set
   028 03/22/99 EAB In process broke ad_base builds so fixed that
   029 02/22/99 EAB Added code to support Wang's closing time work you get this
				if you dont define new_tilt and define closing_time
 * 021 12/04/99	mfg shutoff LOWCOMPUTE for Windows CE
 * 022 03/13/00	mfg turned of AD_BASE for MSDOS build
 *
 *
 *
 ***********************************************************************
...
/* The tuning files vary based on language and whether or not anything has changed
 as literally chnging anything could cause a new tuning to be needed
...
#ifdef ENGLISH //Can't use NEW_NOISE on GERMAN OR SPANISH until it's been verified.
//English implies that ad_base is true
...
//To use wnags's closing time model  define  closing_time
