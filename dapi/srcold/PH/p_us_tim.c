...
//helpme
	...
	for (nphon = 0; nphon < pDph_t->nallotot; nphon++)
	{
			...
			&& pDph_t->number_words >= 4)
		{
			...
			/* eab not truq so change it and see what breaks
...
//			printf("ph=%d ",pDph_t->allophons[endcnt]); 
		...
		sonocnt = 0;
...
#else //TOMBUCHLER
			//if(pDph_t->allofeats[nphon]& FISBOUND)
			...
			if ((phone_feature( pDph_t ,pDph_t->allophons[nphon+1]) & FSYLL)
				&& ( (pDph_t->allofeats[nphon+1] & FISBOUND) != FISBOUND))
				...
				
			{
				...
				if((pDph_t->allofeats[nphon] & FISBOUND) == FISBOUND)
