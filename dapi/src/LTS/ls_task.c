...
extern char apres_tiret[];
extern unsigned char premiere[];
extern unsigned char premier[];
extern unsigned char francs[];
extern unsigned char *categ[];
/*#define LS1DEBUG 1*/
...
#endif
#ifdef FRENCH
		if ( tolower((char)llp->l_ch)=='q' && (llp+1)->l_ch=='u' ) 
		{
			pLts_t->wstate = IS_WH;
		}
#endif
...
/*			goto out; */
	  	...
	  	else if (ls_task_wlookup(pLts_t, llp, &spwhword[0]) != NULL)
			{
			pLts_t->wstate = UNK_WH;			
...
void ls_task_categ_gram(LPTTS_HANDLE_T phTTS)
{
	...
	c = tolower ((char)(pLts_t->cword[0].l_ch));
	
	pt = NULL;
	if ( c>='a' && c<='z' ) {
		if (categ [c-'a'] != NULL) 
			pt = ls_task_wlookup (pLts_t, pLts_t->cword, categ [c-'a']);
	} else {
		pt = ls_task_wlookup (pLts_t, pLts_t->cword, categ[26]);
	}
	if (pt!=NULL) 
	{
		//sendpipe ((FRGC << 8) | *pt); 
		...
		while (lp1 != rlp) { /* 3 loops hiv-cv-dc */
			...
			if ( (lflag & HVOWEL) == 0) {
				...
				if (lp2+1==lp1 && lp1->l_ch=='-'
					&& ls_task_wlookup (pLts_t, lp2, apres_tiret) )  {
