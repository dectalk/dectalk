...
static void ElementArbre (PLTS_T pLts_t, short PtND, BYTE *Nd, SHORT *LH, BYTE *PremPhon, short Inc,
...
extern  short 	PtCar;
extern	short	PgCar;
extern	SHORT	IndAlph [];
extern  BYTE	TCod [];
extern  SHORT	InvAlph [];

/* classes */
extern   BYTE	TTUC [];       /* for classe of type 1 */
extern   BYTE	TypClas [];    /* 1, 2, 3, 4, 5 */
extern   SHORT	DebClasse [];  /* Beginning of class depending on type */
extern   char   ChCl [];       /* strings of word of classes */
extern   BYTE	NdCl [];       /* characters for class d or g */
extern   short	LHCl [];       /* pointor for another alternative */

/* instructions */
extern   BYTE	ChPD [];     /* characters for right strings */

extern	 SHORT	PrtDrte [];  /* right part of the rule */
extern	 SHORT	ContG [];    /* left context of the rule */
extern	 SHORT	ContD [];    /* right context of the rule */

extern   BYTE	FinCont [];  /* end of a part of a context (1 : true) */
extern   BYTE	ClOuCh [];   /* string:0, class:1 */
extern   short	EnsOuCh [];  /* index for class or for a string */


extern   BYTE	Nd []; 	     /* characters for left strings */
extern   short	LH [];       /* pointor for an alternative of left string */

extern   SHORT	SNd [];      /* list of rule with same left string */
extern   SHORT	SLH [];      /* rule number */
extern   char	CtG [];      /* grammatical category of left string or -1 */
...
extern   SHORT	TabBasePG []; /* index on Aid of beginning of rule bloc */
...
*/
static short ChargEB (BYTE *Tab, short i)
{
...
//short	N;
//short	PtMax;
static short CompCh (BYTE *Buf1, short PtBuf1, BYTE *Buf2, short PtBuf2, short N, short PtMax)
{
	short	i;
	FLAG	Egal;
