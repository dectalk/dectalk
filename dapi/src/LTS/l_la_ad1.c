 ...
 * ---	-----	-----------		---------------------------------------
 * 01 	MM		03/24/86 		From DECtalk 1.
 * 02   MM		07/17/86		Dr. Quilis's fixes and Brenda's dipthong hacks
 * 03   JDB		07/19/96		Fix problem of "SPA" null pointer crash case in WIN95 
 * 04   GL      09/27/96        Fix the problem of "tr..." words crash.
 * 05	MFG		06/17/98		Copied from l_sp_ad1.c
 * 06   EAB		11/6/98			BATS#802 Fixed syllabification bug for cuidado code agrees with notes but not
 *								native speakers
   07   EAB		11/16/98		ST is NOT a legal onset cluster in Spanish BATS 794
 */
...
extern  short   featb_sp_lts[];
#define   isstress(lsp)   	(LSISSTRESS(lsp))
#define   isbound(lsp)   	(LSISSBOUND(lsp))
...
static  short syllfeatb[TOT_SP_PHONEMES] = {
      ...
      /* [ E_GH ] */   0      | CODA      | RLONSET   ,
      /* [ E_YH ] */   0                  ,
      /* [ E_P ]  */   0   | SCODA   | CODA      | RLONSET   ,
      /* [ E_B ]  */   0      | CODA      | RLONSET   ,
      /* [ E_T ]  */   0   | SCODA   | CODA   | RONSET      ,
      /* [ E_D ]  */   0      | CODA   | RONSET      ,
      /* [ E_K ]  */   0   | SCODA   | CODA      | RLONSET   ,
      /* [ E_G ]  */   0            | RLONSET   ,
      /* [ E_CH ] */   0                  ,
      /* [ E_Y ]  */   0                  ,
      /* [ E_R ]  */   0                  ,
...
static  char   pl_mente[] = {      /* The -mente ending */
      E_M, E_E, E_N, E_T, E_E
      };
...
static int l_sp_ad1_syllword(PLTS_T pLts_t, PHONE * wstart,PHONE * wend)
{
   PHONE *lsp;
   PHONE *endp;
   int   isfinal;
 ...
 *
 */
static PHONE * l_sp_ad1_dosyllable(PLTS_T pLts_t, PHONE * wstart, PHONE * endp,
                    			   int isfinal)
/*
 * 
 */
 ...
 *
 */
static PHONE * l_sp_ad1_fin_coda(PHONE *wstart, PHONE *endp)
{
   PHONE *lsp;
 ...
 *
 */
static PHONE * l_sp_ad1_int_coda(PHONE *wstart, PHONE *endp)
{
   PHONE *lsp;
   PHONE *prev;
   int   phone;
 ...
 *
 */
static PHONE * l_sp_ad1_nucleus(PLTS_T pLts_t, PHONE * wstart, PHONE * endp,
                 				int isfinal)
{
   PHONE    *lsp;
   int      last_phone;
    ...
    */
    ...
    if (mid_phone == E_U         /* Verb infinitive test */
      && last_phone == E_I
      && endp->p_phone == E_R
      && isbound(NEXT(endp)))
    {
   	...
   	if (mid_phone == E_U         /* Past participle test 	 */
      && (last_phone == E_I      /* for -ada, -odo, etc.     */
      || last_phone == E_A
      || last_phone == E_O)
      && endp->p_phone == E_D
      && ((temp = NEXT(endp)->p_phone) == E_O
      || temp == E_A))
    {
		//EAB BATS 802 
      	if (/*isbound(NEXT(NEXT(endp)))
	 		|| */ (NEXT(NEXT(endp))->p_phone == E_S   /* -ados, etc.  */
	 		&& isbound(NEXT(NEXT(NEXT(endp))))))
	 	{
    ...
    */
    if (isfinal         		/* Final syllable                */
      ...
      && ((isbound(endp)      	/* Either vowel is word final    */
      && (lsp->p_phone == E_O   /* and is either "o" or "u"    	 */
      || lsp->p_phone == E_E))
      || (lsp->p_phone == E_O   /* Or word ends in "on"          */
      && endp->p_phone == E_N
      && isbound(NEXT(endp)))))
    {
      /*
 ...
 *
 */
static PHONE * l_sp_ad1_onset(PHONE* wstart, PHONE* endp)
{
   PHONE      *lsp;
   PHONE      *tlsp;
   ...
   if (lsp == wstart && isonset(phone))
      goto at_word_start;
   prev = PREV(lsp);
   if ((phone == E_R || phone == E_L)
      && isrlonset(prev->p_phone))
   {
      if (prev->p_phone == E_P && prev != wstart && l_sp_ad1_sonset(prev))
	  { 
   ...
   {
      if (prev->p_phone == E_T && l_sp_ad1_sonset(prev))
 ...
 *
 */
static int l_sp_ad1_sonset(PHONE *lsp)
{
...
//   if (!lsp || !(PREV(lsp)) || !(PREV(PREV(lsp))))
   ...
   if ((lsp->p_phone) == E_S
	  && (temp  != E_T) //EAB 11/16/98 ST is NOT a legal onset cluster in Spanish BATS 794
	  && (temp  != E_P) //EAB 11/16/98 ST is NOT a legal onset cluster in Spanish BATS 794
      && !(temp = PREV(PREV(lsp))->p_phone) != E_N
      && !isvowel(temp))
      return (TRUE);
   return (FALSE);
}
 ...
 *
 */
static PHONE * l_sp_ad1_wordstart(PLTS_T pLts_t, PHONE * wstart, PHONE * endp)
{
   int   phone;
...
void l_sp_ad1_stress(PHONE * wstart, PHONE * wend, int syllcount)
{
   ...
   if ((lsp->p_phone == E_S || lsp->p_phone == E_N)
      && isvowel(PREV(lsp)->p_phone))
   {
