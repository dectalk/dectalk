...
int lsa_util_id_name(LETTER *llp,LETTER *rlp)
{
	...
	for(i=0;llp < rlp;i++)
		{
		name[i] = (char)(*llp).l_ch; // NAL warning removal
		if((ls_char_feat[(int)name[i]] & (CFEAT_vowel|CFEAT_cons)) == 0)
			 return(NAME_ENGLISH);
		llp++;
		}
	name_size = i;
		...
		for(i=name_size-1;i>=0;i--)
			...
			switch(er[off]) {
				...
				case    'C' :
					...
					if(ls_char_feat[(int)name[i]] & CFEAT_cons)
						nm += 1;
					else if(nm)
						{
						i += 1;
				...
				case    'V' :
					...
					if(ls_char_feat[(int)name[i]] & CFEAT_vowel)
						nm += 1;
					else if(nm > 0 && nm <=2)
						{
						i += 1;
