...
void ls_rule_lts(LPTTS_HANDLE_T phTTS,LETTER *llp, LETTER *rlp,int def_lang, int sel_lang)
{
	...
	while (llp < rlp) 
	{
		...
		if (lch>='a' && lch<='z')
		{
                        if (ls_rule_add_graph(pLts_t,gp1, lch-'a'+GGA) != FALSE)
				++gp1;
		} 
		else 
		{
            ...
            {
				case '\'': 
                    if (ls_rule_add_graph(pLts_t,gp1, GGQUOTE) != FALSE)
						++gp1;
                    break;                        
		       	case 228: /* for ä */
				case -28:
					if (ls_rule_add_graph(pLts_t, gp1, GGAx) != FALSE)
				  	++gp1;
					break;
				case 246: /* for ö */
				case -10:
					if (ls_rule_add_graph(pLts_t, gp1, GGOx) != FALSE)
					++gp1;
					break;
		    	case 252: /* for ü */
				case -4:
					if (ls_rule_add_graph(pLts_t, gp1, GGUx) != FALSE)
				    ++gp1;
					break;
		    	case 223: /* for ß */
		      	case -33:
					if (ls_rule_add_graph(pLts_t, gp1, GGSx) != FALSE)
				   	++gp1;
					break;       
			}
		}
	...
	while (pp1 != &pLts_t->phead)
	{
		...
		if ((pp1->p_flag&PFMORPH) == 0)
		{
			...
			if (ph1==ph2 && ls_adju_is_cons(pp1)!=FALSE)
			{
				...
				if (ph2 == F || ph2 == L || ph2 == M || ph2 == N || ph2 == P || ph2 == T)
				{
 ...
 *
 */
int ls_rule_add_graph(PLTS_T pLts_t, struct GRAPH_struct *gp, int g)
{
	int     g1;
	PHONE   *pp;
	int     value;
