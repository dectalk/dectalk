 ...
 *	025 NAL		04/02/2000		Added support for linux demos
 *****************************************************************************/
...
int linux_get_dict_names(char *main_dict_name,char *user_dict_name);
...
void GetDictionaryNames( char *, char *, char * );
...
#ifdef SPANISH_LA
		nDicLoad = load_dictionary( phTTS,
						&(pKsd_t->fdic[LANG_latin_american]),
						&(pKsd_t->fdic_entries[LANG_latin_american]),
						szMainDict,
						TRUE,
						TRUE );
//		nAdicLoad = load_dictionary( phTTS,
//						&(pKsd_t->adic[LANG_latin_american]),
//						&(pKsd_t->adic_entries[LANG_latin_american]),
//						szAbbrDict,
//						TRUE,
//						TRUE );
#endif
...
#endif
#ifdef SPANISH_LA
		gpufdic = pKsd_t->fdic[LANG_latin_american];
		gufdic_entries = pKsd_t->fdic_entries[LANG_latin_american];
#endif
#ifdef GERMAN
...
#endif
#ifdef SPANISH_LA
		pKsd_t->fdic[LANG_latin_american] = gpufdic;
		pKsd_t->fdic_entries[LANG_latin_american] = gufdic_entries;
#endif
#ifdef GERMAN
...
#ifdef SPANISH_LA
nDicLoad = load_dictionary( phTTS,
				  &(pKsd_t->udic[LANG_latin_american]),
				  &(pKsd_t->udic_entries[LANG_latin_american]),
				  szUserDict,
				  FALSE,
				  TRUE );
#endif
...
nDicLoad = load_dictionary( phTTS,
  ...
  */
  ...
  linux_get_dict_names(main_dict_name,user_dict_name);
...
#ifdef ENGLISH_UK
#ifdef __arm__
nDicLoad = load_dictionary( (void **)&(pKsd_t->fdic[LANG_british]),
  &(pKsd_t->fdic_entries[LANG_british]),
...
#endif
#ifdef SPANISH_LA
    nDicLoad = load_dictionary( &(pKsd_t->fdic[LANG_latin_american]),
		   &(pKsd_t->fdic_entries[LANG_latin_american]),
		   main_dict_name,
		   TRUE );

//    nAdicLoad = load_dictionary( &(pKsd_t->adic[LANG_latin_american]),
//		   &(pKsd_t->adic_entries[LANG_latin_american]),
//		   "/usr/lib/dtk/abbr_la.dic",
//		   TRUE );
#endif
#ifdef GERMAN
...
//		   TRUE );
    ...
    nDicLoad = load_dictionary( &(pKsd_t->fdic[LANG_french]),
		   &(pKsd_t->fdic_entries[LANG_french]),
		   main_dict_name, PLEASE_BLOW_UP_NOW!!!!
		   TRUE );
...
#endif
#ifdef SPANISH_LA
        load_dictionary( &(pKsd_t->udic[LANG_latin_american]),
        		   &(pKsd_t->udic_entries[LANG_latin_american]),
        		   user_dict_name,
        		   FALSE );
#endif
...
#ifdef UNDER_CE	
  TCHAR wszMainDict[MAX_STRING_LENGTH/2]; // wide char type needed for windows CE mfg 01/06/1999
  TCHAR wszUserDict[MAX_STRING_LENGTH/2]; // wide char type needed for windows CE mfg 01/01/1999
#endif
