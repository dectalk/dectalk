...
#else
		if((FDICT_ENTRY != 0) && (ls_dict_ufind_word(phTTS,2) == HIT))
#endif
		{
			return(pLts_t->hit_type);
		}
...
#else
			if(ls_dict_ufind_word(phTTS,2) == HIT)
#endif
			{
				/* restore the original word structure */
				pLts_t->comp_str[i] = 0;
...
MMRESULT DumpDictionary(LPTTS_HANDLE_T phTTS, char *filename)
{
	...
	for (lcv = 0; lcv < DICT_ENTRY; lcv++)
	{
		...
		if (DICT_ACCESS(lcv)->fc)
		{
			fc_mask = 1;
			for (lcv2 = 0; lcv2 < 32; lcv2++)
			{
				if (DICT_ACCESS(lcv)->fc & fc_mask)
						   ...
						   unsigned char *new_phoneme)
{
	...
	entry->fc = UDICT_ACCESS(index)->fc;
