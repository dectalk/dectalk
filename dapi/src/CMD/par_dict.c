...
int _far par_dict_lookup(PKSD_T pKsd_t,unsigned char *word, int in_flag)
{
	...
	if( (ls_sapi_find_word(pKsd_t->phTTS, word, 1) == HIT)||
		(flag == 1 && (ls_sapi_find_word(pKsd_t->phTTS, temp_word, 1) == HIT)) )
	{
		return HIT;
	}
#endif
	...
	if (UDICT_ENTRY != 0 && ((par_dict_ufind_word(UDICT_ENTRY,UDICT_INDEX, UDICT_DATA,word) != MISS) ||
		(flag == 1 && par_dict_ufind_word(UDICT_ENTRY,UDICT_INDEX,UDICT_DATA,temp_word) != MISS)))
	{
		/* debug switch */
	...
	if (FDICT_ENTRY != 0 && ((par_dict_ufind_word(FDICT_ENTRY,FDICT_INDEX, FDICT_DATA,word) != MISS) ||
		(flag == 1 && par_dict_ufind_word(FDICT_ENTRY,FDICT_INDEX,FDICT_DATA,temp_word) != MISS)))
	{
		/* debug switch */
		if (DT_DBG(CMD_DBG,0x020))
		{
	...
	if (DICT_ENTRY != 0 && ((par_dict_find_word(DICT_ENTRY,DICT_INDEX,DICT_DATA,word) != MISS)||
	   (flag == 1 && par_dict_find_word(DICT_ENTRY,DICT_INDEX,DICT_DATA,temp_word) != MISS)))
	{
		/* debug switch */
		if (DT_DBG(CMD_DBG,0x020))
		{
...
/* Use the correct KS dic entry based on language being compiled */
	...
	do
	{
		...
		if ( (stat=par_dict_dlook(DICT_ENTRY,DICT_INDEX,DICT_DATA,base,&pent,word)) == HIT)
			break; /* found it. */
