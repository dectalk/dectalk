 ...
 * 056	GL		20nov98		BATS#828 use CMD_DEBUG to replace _DEBUG
 * 057  MFG		08jan98		WINprintf not supported under Windows CE
 * 058  GL		03Mar1999	BATS#676,697 and 744 fix the coding error
...
//			if ((pCmd_t->clausebuf[pCmd_t->input_counter-2] == '.') && (pCmd_t->ParseChar== 0x0fff) &&
		    ...
		    (par_dict_lookup(pKsd_t,cm_text_get_word(pCmd_t->prevword,pCmd_t->wordbuf,1),0)))
			{
//printf("*D %x\n",pCmd_t->ParseChar);
				pCmd_t->done=0;
			}
		  ...
		  if ((pCmd_t->skip_mode != SKIP_email) &&
			...
			if (pCmd_t->skip_mode != SKIP_punct)
			{
				...
				for (i=0;i<pCmd_t->input_counter;i++)
				{
					...
					{
						// MGS		10/14/1999		BATS#876 fix for UK phone numbers (part of it) 
						pCmd_t->dict_hit_buf[i]=par_dict_lookup(pKsd_t,cm_text_get_word(&(pCmd_t->clausebuf[i]),pCmd_t->wordbuf,0),1);
...
/* GL 04/21/1997  change this for OSF build */
			...
			if (pCmd_t->skip_mode != SKIP_rule)
			{
				...
				// Add protection code for buffer overflows
				pCmd_t->clausebuf[PAR_MAX_INPUT_ARRAY-1]='\0';
	        	pCmd_t->input_counter=strlen(pCmd_t->clausebuf);
				for (i=0;i<pCmd_t->input_counter;i++)
				{
