...
void cm_cmd_build_param(LPTTS_HANDLE_T phTTS, unsigned int c)
{
	...
	switch (FORMAT_PARAM) 
	{
		...
		case 'd':
			...
			{   
				/* 
				 * a minus is found; set pCmd_t->p_flag to TRUE to signal
		...
		case 'a':			
			...
			if(pCmd_t->q_flag && c != pCmd_t->q_flag)
			{
	...
	/*
	 *  now, pCmd_t->p_flag may be set to cause a negation, if so do it ...
	 */

	if(pCmd_t->p_flag == TRUE)
	{
