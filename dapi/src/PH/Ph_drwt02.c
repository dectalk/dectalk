 ...
 * 0022	EAB	9/29/98	EAB		Refining and tuning the new intonation lot's of small
					changes 
	End old comments
 ...
 *  0034  EAB		04/6/98		Oliver made me do it....I tuned the German intonation, in general he feels if should be quite flat so we're
	...
	0041  EAB		9/11/98		Evaluated new code base and made fixes and improvements, more work to be done.
    0042  EAB 10/16/98 Tuned for new release of all languages on new_intonation platform.
	0043  EAB Putting ENGLISH_UK onto this development base
	0044 EAB Refinesd F1 F0 relationship 
	0055 EAB 	//eab 10/20/98 Reduce effect further for Spanish (f1 f0)
	0056 EAB 	0044 EAB 11/9/98 UK_English male and female voice BATS 777
					if (new_intonation enabled for UK) This as part of the overall 5.0
				tuning process file in AD already contains changes 
    057 EAB 	2/23/99 Added in copyright plus new gov wording plus removed incorrect
						time adjustment(found Bill Hallhan had added comp in
						vtm
 *  056  EAB		5/3/99  Fixed BATS 887
 *	057	 MGS	12/17/1999	Fix for SAPI symbol conflict
 *  058 EAB     4/17/00   Many tuning changes for 4.6 release 
 */
...
if (pDph_t->nf0ev <= -2) 
	{
		...
		pDph_t->f0basetypes[1][13]=1100;
		pDph_t->f0basetypes[1][14]=1100;
		pDph_t->f0basetypes[1][15]=1100;
...
/* DECLARATIVE*/
		...
		pDph_t->f0basetypes[0][7]=1082;
		pDph_t->f0basetypes[0][8]=1079;
		pDph_t->f0basetypes[0][9]=1075;
		pDph_t->f0basetypes[0][10]=1062;
		pDph_t->f0basetypes[0][11]=1059;
		pDph_t->f0basetypes[0][12]=1045;
		pDph_t->f0basetypes[0][13]=1032;
		pDph_t->f0basetypes[0][14]=1029;
		pDph_t->f0basetypes[0][15]=1015;
		pDph_t->f0basetypes[0][16]=1002;
		...
		/*eab  less drop for short phrases*/
			...
			pDph_t->f0basetypes[4][13]=1040;
			pDph_t->f0basetypes[4][14]=1030;
			pDph_t->f0basetypes[4][15]=1010;
			pDph_t->f0basetypes[4][16]=990;
...
//	WINprintf("male %d %d \n", pDphsettar->glide_tot, pDph_t->allophons[pDphsettar->np_drawt0]);
		...
		/* nEED TO DETERMINE HOW TO DECREASE THIS*/
		if((pDphsettar->glide_inc == 0 )
			&& pDphsettar->glide_tot > 0)
		{
			pDphsettar->glide_tot -=30;
		}
#endif //german
...
/*	  end of segment (98% per frame)  */
	...
	vocal tract postition as implemented this effect is quite weak (better safe than sorry) no time for a long
			detailed evaluation but this should add some positive variability eab 6/13/98 wrong sign*/
			/* olivertest */
				//eab 10/20/98 Reduce effect further for Spanish 
#ifdef SPANISH
			temp = ((pDph_t->parstochip[1] ));
			f0in +=(350-temp)>>2;
#else
			temp = ((pDph_t->parstochip[1] ));
			f0in +=(350-temp)>>1;
	    ...
	    filter_seg_commands(pDph_t,f0in);	/* Input is pDphsettar->tarseg1, output is pDphsettar->f0s */
...
else
{
	 if (pDph_t->nf0ev <= -2) 
	{
		...
		pDph_t->f0basetypes[1][15]=1744; 
		pDph_t->f0basetypes[1][16]=1738; 
...
/*    Add f0 drift or flutter (approx 3 and 5-Hz sine waves, each +/-1.0 Hz) */
	if (pDph_t->f0mode< SINGING) {
		...
		if(pDph_t->clausetype == 2)
		{
			pDph_t->f0prime += mlsh1(pseudojitter,pDph_t->arg2);
			pDph_t->addjit = -pDph_t->addjit;
		}
