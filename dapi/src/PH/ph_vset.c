...
void setparam (LPTTS_HANDLE_T phTTS, int which, int value)
{
	...
	if (which < SPD_SEX || which > SPD_GS) /* changed upper limit from SPD_SR to SPD_GS, JAW
		                                    4/27/98 */
	{								   /* Check for junk. */
		return;
	}
...
/* BATS 711 MOve German to the new method for filter calculation*/
	...
	pDph_t->spdefglspeed = pDph_t->curspdef[SPD_GS];
...
#else
#ifdef NEW_VOLUME
	spdef->apgain = pDph_t->curspdef[SPD_GH]-temp;	/* GH -> GH         */
#else
	spdef->apgain = pDph_t->curspdef[SPD_GH];	/* GH -> GH         */
#endif
#endif
