...
{
#ifdef DEBG
	//if (dbgdur) printf ("p_fr_tim.c  regle() %4x %s\n", 
	if (dbgdur) printf ("p_fr_tim.c  regle() %2s %s\n",  // Sorry, I need the name
		phprint (phocur), mes);
#endif
...
short Allonge_Cons_Finale (PDPH_T pDph_t, short Seg_Prec, short Cons, short pcnt)
{
...
#ifdef DEBG
    //if (dbgdur) printf ("p_fr_tim.c  Allonge_Cons_Finale entree %4x [%4x] pcnt = %d\n",
  ...
  {	
    ...
    case F_V: case F_Z: case F_J: 
    	...
    	if( phone_feature (pDph_t, Seg_Prec) & TNasale || 
        	phone_feature (pDph_t, Seg_Prec) & TArrondie )
	 	{
      		if (Cons&PVALUE==F_J)
...
#define Fact_Elongation       PCent6
  ...
  Syll_Synt_Restantes   = (pDph_t->allofeats [0] & 0xFFFF) >> 8;  /* Lg 1er syntagme */
  ...
  for (nphon=0; nphon<pDph_t->nallotot; nphon++) {
    ...
    if (nphon+1<pDph_t->nallotot) pDphsettar->phonex_timing = pDph_t->allophons [nphon+1]; else pDphsettar->phonex_timing = GEN_SIL;
    ...
    if (pDph_t->user_durs[nphon] != 0) {
      ...
      case GEN_SIL & PVALUE :  /* pDphsettar->durxx : number of frames */
        if (pDph_t->allofeats [nphon] & FPROP) { /* end of the sentence */
          pDphsettar->durxx =  NF_PERIOD + pDph_t->perpause;  /* phdefs.h 75 frames */
        } else {
          pDphsettar->durxx = NF_COMMA + pDph_t->compause;  /* 16 frames */
        }
      ...
      case F_ON: case F_OU: case F_U:  case F_UN:
        ...
        if (FMot (pDph_t, nphon, 1) ||
          phone_feature (pDph_t, pDphsettar->phonex_timing) & TVoyelle ) {
          /* DAu_, A of BOA_, An of BRUAn_
             O of BOA, U of BRUAn */
          regle (phocur, "[vowel].(End of word) or [vowel].(vowel), coeff: 1");

#ifdef DEBG
            if (dbgdur) printf ("p_fr_tim.c  phtiming()  D_Base %d\n", D_Base);
#endif

        } else {
          FLAG  Voy_Fermee;     /* True if high vowel */
          FLAG  Voy_Ouverte;    /* True if low  vowel */

          if (phone_feature (pDph_t, phocur) & TFermee) {
            Voy_Fermee = TRUE;
          } else {
            Voy_Fermee = FALSE;
          }
          if (phone_feature (pDph_t, phocur) & TOuverte) {
            Voy_Ouverte = TRUE;
          } else {
            Voy_Ouverte = FALSE;
          }
...
#ifdef DEBG
            if (dbgdur) {
              printf ("p_fr_tim.c exDuree_Voyelle  pcnt %d, Voy_Fermee %d, Voy_Ouverte %d\n",
                pcnt, Voy_Fermee, Voy_Ouverte);
            }
#endif

          if ( phone_feature (pDph_t, phocur) & TNasale ) {    /* [An,In,On,Un] */
            if ( !(phone_feature(pDph_t, pDphsettar->phonex_timing) & TPlosive) 
                || phone_feature(pDph_t, pDphsettar->phonex_timing) & TVoisee ) {
              /* [An,In,On,Un].(fricatives ou nasals or B, D, G) */
              if (FMot (pDph_t, nphon, 2) || SegD2 == GEN_SIL || SegD2 == FP_AP) {
                /* t[On]B or T[On]BAp, m[AN]j, l[ON]j, b[ON]z, v[AN]v */
                pcnt += PCent15; 
                regle (phocur, "[An,In,On,Un].(not plosive or voiced plosive).(EOW or Ap):+15%");
              }
            }
            // otherwise default value for nasal vowels
            // BL[An]ChI, K[An]PAGn, L[On]TAn, [In]KONU, P[An]SE1, DE1S[An]DAn 
            regle (phocur, "[An,In,On,Un]: normal coeff 1.00");
...
#if 0
            case F_R:
              if (!FMot (pDph_t, nphon, 2) && phone_feature(pDph_t, SegD2) & TVoyelle ) {
            ...
            case F_L:
              if (!FMot (pDph_t, nphon, 2)) {
                if ( !(phone_feature(pDph_t, SegD2) & TVoisee) ) {
            ...
            case F_J: case F_V: //case F_Z:
              //R[E2]V, R[OU]JATR, JAR[I]VERE2 
              //not KRW[A]ZE1 */
              pcnt += PCent15;
              regle (phocur, "[vowel].(J,V,Z)  +15%");
              break;

            case F_W: case F_WU: case F_Y:
              if (Voy_Ouverte) {
                if ( FMot (pDph_t, nphon, 2) || phone_feature(pDph_t, SegD2) & TVoisee ) {
                  // V[O6]Y, [A]Y, [A]Y06R
                  pcnt += PCent20;
        ...
        /* Modifications for stressed or unstressed words depending on 
           the remaining number of vowels in the clause or in the word */
        if (!Mot_Accentue) { // unstressed word : reduce the vowel
          pcnt = mlsh1 (pcnt, N70PRCNT);
          regle (phocur, "Unstressed vowel 0.70");
        } else if (Syll_Synt_Restantes == 0) { // Last vowel of the clause (word stressed)
          if ( (phocur != FP_E) && (phocur != FP_AP)) {
            if (FMot (pDph_t, nphon, 1) ) {
              if (phocur == FP_E1 || phocur == FP_E2 || phocur == FP_U) {
                // phocur; = last phoneme (vowel) in clause (no consonant after)
                // PAnS[E1]. KRWAZ[E1], COURB[E1],FOR[E2]. JARIVR[E2]
                // INKON[U], VWAT[U]
                pcnt = mlsh1 (pcnt, N140PRCNT);
                regle (phocur, "[E1, E2], last vowel in clause (no consonant after) 1.40");
              } else {
        ...
        } else if (Syll_Mot_Restantes == 0) { // the last vowel of the stressed word
          // default coeff : 1 //D[E2] LAuB, BLAnCh[I], [A], L[A]
          regle (phocur, "The last vowel of the word coeff: 1");
        } else if (FMot (pDph_t, nphon, 0)) { // the first phoneme of the stressed word
          // [Au]KUn, [In]KONU, [A]RFLO6R
          pcnt = mlsh1 (pcnt, N80PRCNT);
          regle (phocur, "The first phoneme (vowel) of the word 0.80");
        } else { // not the first vowel, not the last one of a stressed word
          // D[E]MIn, BL[An]ChI
          pcnt = mlsh1 (pcnt, N80PRCNT);
          regle (phocur, "Not the first phoneme, not the last vowel 0.80");
        }
      ...
      case F_Y:  case F_Z:
          ...
          case F_K: 
            switch (pDphsettar->phonex_timing & PVALUE) {
            case F_S: case F_T: // FIKSE1, VIKTOR
              pcnt = mlsh1 (pcnt, N70PRCNT);
        ...
        /* Modify consonant duration depending on the position in the clause 
           and in the word */
        if (Mot_Accentue && (Syll_Synt_Restantes == 1)) {
          /* consonant(s) in the syllable before the last one */
          // default coeff = 1 // DE[M]In, MA[T]An, BLAn[Ch]I
          regle (phocur, "The consonant(s) before the last vowel of clause 1.00");
        } else if (Mot_Accentue && (Syll_Mot_Restantes == 1)) {
          /* consonant(s) in the syllable before the last one */
          // default coeff = 1 // BLAn[Ch]I, [V]WAR
          regle (phocur, "The consonant(s) before the last vowel of a stressed word 1.00");
        } else if (!Mot_Accentue) { // word not stressed: reduced every consonant
          pcnt = mlsh1 (pcnt, N80PRCNT);
        ...
        } else if (FMot (pDph_t, nphon, 0)) { // the first consonant of the word
          //pcnt = mlsh1 (pcnt, N90PRCNT); // [K]RWAZE1, [S]ERA
          regle (phocur, "The first consonant of the word 1.00");
        } else { // Not the first nor the final consonant(s) of the word
      ...
      if (phocur != GEN_SIL) {    /* already in frames */
        ...
        if (Fin_Groupe && (Syll_Mot_Restantes==0) && (Syll_Synt_Restantes != 0)) {
          if ( phone_feature (pDph_t, phocur) & TVoyelle ) {
            ...
            if ( phone_feature (pDph_t, phocur) & TPlosive ) { //'V'.[P,T,K,B,D,G]._
              if ( (phone_feature (pDph_t, pholas) & TVoyelle) && 
                   FMot (pDph_t, nphon, 1) ) {
                pcnt = mlsh1 (pcnt, N120PRCNT);
                regle (phocur, "Last consonant (plosives) of a group  1.20");
              }
            } else if ( (phone_feature (pDph_t, pholas) & TVoyelle) && 
                        FMot (pDph_t, nphon, 1) ) {
