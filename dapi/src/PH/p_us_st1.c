 ...
 *  008 EAB		5/23/98		The BATS660 found a problem with the level of the silenced glotal
 *							stop in SLOWTALK in US ENGLISH too
 *  009 EAB		5/18/98  	BATS249 removed unused variable pholm2.
 ...
 *  011 EAB 2/3/99  Added in code suggested by Dennis including supplyinfg a weak burst for df and th
 *  012 EAB 3/26/99 EAB BATS 878 Fixed the burst-plosive and plosive-plosive problem 
 *     
 */    
...
static short gettar (LPTTS_HANDLE_T phTTS, short nphone_temp)
{
	...
	else if (pDphsettar->par_type IS_NASAL_ZERO_FREQ)
	{
		...
		{
			tartemp = NASAL_ZERO_ALVELAR;			   /* Value dur nasal murmur (527=max) */
		}
				   			   ...
				   			   short feanex, short strucnex)
{
	...
	//debugfackward ("Set default", pDphsettar->np);
	...
	if (pDphsettar->par_type IS_FORM_FREQ)
	{
		...
		else
		{
			...
			{
				//debugfackward (" Use locus,percent", pDphsettar->np);
			}
	...
	else if (pDphsettar->par_type IS_FORM_BW)
	{
		...
		{
			pDphsettar->durtran = 0;
			//debugfackward (" Nasal has const. par val", pDphsettar->np);
		}
	}
			 ...
			 || (pDphsettar->par_type IS_AV_OR_AH))
	{
		...
		if (pDphsettar->bouval < temp)
		{
			...
			//debugfackward (" Onset", pDphsettar->np);
		}
		...
		{
			pDphsettar->durtran = 0;
			//debugfackward (" Make next onset abrupt", pDphsettar->np);
			...
			if (((feacur & FPLOSV) IS_PLUS) || (pDphsettar->phcur == CH))
			{
				if ((feacur & FVOICD) IS_PLUS)
				{
					...
					pDphsettar->durtran = NF45MS;
					//debugfackward (" Voicebar dies out", pDphsettar->np);
				}
		...
		if ((feacur & FNASAL) IS_PLUS)
		{
			...
			{
				pDphsettar->durtran = 0;
				//debugfackward (" Make murmur const AV", pDphsettar->np);
			}
			else
			{
				pDphsettar->durtran = NF40MS;
		...
		if (pDphsettar->np == &PAV)
		{
			...
			//debugfackward (" Offset to noise", pDphsettar->np);
		}
		...
		{
			pDphsettar->durtran = 0;
			//debugfackward (" Stop closure next", pDphsettar->np);
		}
...
*/
		...
		if ((pDphsettar->phcur == TH) || (pDphsettar->phcur == DH)) 
		{
