 ...
 * 0030 EAB		3/30/1998		Uncompleted changes for new GERMAN
 * 0031 JAW     04/27/1998      Added default glottal speeds.
 * 0032 EAB		4/27/1998		Preliminary tune for GERMAN submission.
 * 0033 EAB		4/29/1998		Fast manual tune for submission and wave file
								WARNING 8K VOICES ARE NOT TUNE AT ALL
 * 0034 EAB		4/29/1998       Readjust some gains
 * 0035 EAB		6/3/1998		Tuned 8K voices except for floating point model gain using cooledit
 ...
 * 0036 MGS+EAB	06/4/1998		Tuned 8 kHz voices for the floating point
 * 0037 EAB		7/8/1998		BATS 709-will all need gain adjust before ship
 * 0038 EAB		7/16/1998		Gain adjust for rnd submital to Carl both 10kha and 8 khz, also move 
 ...
 * 0041 EAB 					Set fricative and aspiration balance 5/2/00
 * 0042 CAB		10/16/2000		Changed copyright info
 * 0043 EAB 	03/13/2001 		Modify for 4.61 release
 * 0044 CAB		03/16/2001		Updated copyright info
 */
...
short paul[SPDEF] =
{
	...
	260 ,						       /* B4 (was 230, bandwidth in Hz of cascade 4th formant) */
	4650 ,						       /* F5 (was 3900, frequency in Hz of cascade 5th formant = F5*100/HS) */
	330,						       /* B5 (was 180, bandwidth in Hz of cascaded 5th formant) */
	3350,							   /* F7 (frequency in Hz of parallel 4th formant = F7) */
	3850,							   /* F8 (frequency in Hz of parallel 5th formant = F8) */
	65,								   /* GF (gain of frication source in dB) */
	...
	65,								   /* GV (gain of voicing source in dB ) */
	64,								   /* GN (gain of input to cascade nasal pole pair in dB) */
	...
	59,								   /* FT (f0-dependent spectral tilt in % of max) */
	18,								   /* BF (baseline f0 fall in Hz) */
	90,								   /* LX (lax folds adjacent to voiceless sound -> breathiness) */
	40,								   /* QU (quickness of larynx gestures in % of max quickness) */
	9,								   /* HR (hat-pattern fundamental frequency rise in Hz) */
...
short                   betty[SPDEF] =
{
	FEMALE,							   /* SEX = f */
	70,								   /* SM (smoothness in %, actually spectral tilt offset) */
	95 ,								   /* AS (assertiveness, degree of final f0 fall in % */
...
short                   ursula[SPDEF] =
{
	FEMALE,							   /* SEX = f */
	40,								   /* SM (smoothness in %, actually spectral tilt offset) */
	105,							   /* AS (assertiveness, degree of final f0 fall in % */
	...
	59,								   /* G3 (gain of input to cascade 3rd formant in dB) */
	61,								   /* G4 (gain of input to cascade 2nd formant in dB) */
	58,								   /* LO (Loudness, gain input to cascade 1st formant in dB) */
...
short                   rita[SPDEF] =
{
	...
	110,								   /* AS (assertiveness, degree of final f0 fall in % */
