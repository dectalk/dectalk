...
#else
static char             allo_bits[LA_TOT_ALLOPHONES - (LAP_RR & PVALUE)] =
#endif
{
	  /* rr     l       ll      m       n       nh                      */
		 0, 	0, 		0, 		KG + 0, KG + 0, 0,
...
static void spanish_allophonics (LPTTS_HANDLE_T phTTS)
/* 
...
#ifdef OUTPERJUAN //EAB 6/22/98 This needs to be researched I remember it with anna
			case LAP_R:
...
void get_stress_of_conson (LPTTS_HANDLE_T phTTS,short msym, short compound_destress) 
{
	short                   m, stresslevel;
/*	short                  *mp; */
...
void find_syll_to_stress (LPTTS_HANDLE_T phTTS,short *n,short nstartphrase)
{
	...
	if (scnt == 1)
		insertphone (phTTS,(short)saveit, S1);  /* Insert S1 at location m */
	return;
}
