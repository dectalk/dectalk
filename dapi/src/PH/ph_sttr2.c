...

{
	...
	short                   sontyx, ploc, locus, prcnt, delta_freq,tmp;
	...
	{
		ploc = plocu( (fonobst + (FR_TOT_ALLOPHONES * (sontyx - 1) ) ) );
...
#endif
#ifdef FRENCH
	if (((phone_feature( pDph_t, fonsonor) & TVoyelle) IS_PLUS)
		&& ((phone_feature( pDph_t, fonvowel) & TVoyelle) IS_PLUS)
		&& (pDphsettar->np == &PF2))
#endif
{
