 ...
 *  017 EAB		8/5/98      BATS 734
 *  018 EAB					Reuduce aspiration release levels
 *  019 EAB 	3/26/99  	BATS 878 Fixed the burst-plosive and plosive-plosive problem 
	020 EAB		6/7/47 		Modify to support Nasal value definitions
 ...
 *  023 EAB		06/28/2000	Unified Phoneme Set Changes
 *  024 CAB		10/16/00	Changed copyright info and edited mistakes in comment
 *	025	MGS		12/13/2000	Further optimization on gettar
 *	026	MGS		01/18/2001	Removed arg1 arg2 and arg3 from calculations
 *      027  	2/28/01 Eab	lower h aspiration for H.
 *      028     EAB	Fix problem in glottal stop insertion.
 *	029 EAB 3/15/01 Part of fix for voiced unvoiced h variations
...
short gr_gettar (LPTTS_HANDLE_T phTTS, int nphone_temp)
{
	...
	if( PFGR != pDph_t->last_lang)
	{
		pDph_t->last_lang = PFGR;
		...
		*/
		...
		else
		{
			...
			if (phone_temp == GRP_H )
			{
				...
				if (begtyp(phnex_temp) != 1)
				{
					tartemp = 63;	   /* Stronger asp before +back */
				}
			}
...
groupings" */
		 ...
		 && ((feacur & FSONOR) IS_PLUS)))
	{
		...
		if (gr_begtyp[pDphsettar->phonex] != 1)
		{
