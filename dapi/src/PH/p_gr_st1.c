...
*   018 EAB EAB 3/26/99 EAB BATS 878 Fixed the burst-plosive and plosive-plosive problem 
	019 EAB		6/7/47 Modify to support Nasal value definitions
*/
...
static short special_coartic (PDPH_T pDph_t, short nfon, short diphpos)
{
	...
	if (pDphsettar->np == &PF2)
	{
		...
		if (fonnex == L)
		{
			if ((foncur == E) && (foncur == AE) || (foncur == I)
				|| (foncur == EH)
				|| (foncur == AEH)
				|| (foncur == IH)
				)
