 ...
 *  002 EDB		12/10/1996  Many changes for bug fix.
 *  003 EDB		01/10/1997	Fix many data.
 * 	004 EAB     2/6/97      Isolated problem that was causing spa and pa to have the same VOT time.
 *  005 EDB		10/19/1997	use phTTS handler to replace pDph_t
 *  006 EAB		1/17/98     Fixed MAJOR BUG in malamp lookup if the begtynex was six it reached over and
                            grabbed the wrong phoneme
	007 EAB		3/2/98		At least temporarily reduce aspiration for [ n oe ]it's too hot
	008 EAB		3/20/98		Several modifications to improve the German these are commented seperately
							and identified as improv330
*   009 EAB		4/4/98		Found problem where stess was modifying aspiration aas well as av because it was in
*							the wrong place.
*	010 EAB		4/10/98		Removed WINprint from fatal error exit
*	011 EAB		4/27/98		Tuned many values with Oliver for German
*	012 GL		5/04/98		use dt_abort to replace exit(1)
*   013 EAB		5/13/98		Added slowtalk code BATS660
*   014 EAB		5/18/98  	BATS249 removed unused variable pholm2.
*   015 EAB	5/20/98     BATS681  Remove unneeded code--debugforward && debugbackwards
*	016	EAB		7/8/98		BATS 709
*   016 EAB		8/5/98      BATS 734
*   017 EAB	Reuduce aspiration release levels
*/
...
/* printf("tartem1 %d of %d \n",*tartemp,phone_temp); */
	...
	else if (pDphsettar->par_type IS_NASAL_ZERO_FREQ)
	{
		...
		case L:
			/* tartemp = 527;*/
			/*
			 * eab 5/95 can't make flat f1 otherwise pole zero pair can't 
			 * be 100% correct 
			 */
			break;
		case AN:
	...
	else if (pDphsettar->par_type IS_AV_OR_AH)
	{
	   ...
	   /* Reduce av at end of cluase needs to be prerceptually tested eab 3/96 
		*/
		else if ((pDph_t->nallotot - pDph_t->nphone) < 2 & tartemp > 50)
		{
			tartemp = tartemp - 1;
		}
		...
		else
		{
			...
			if (phone_temp == H )
			{
				...
				if (begtyp[phnex_temp] != 1)
				{
					tartemp = 67 - 5;	   /* Stronger asp before +back */
				}
			}
			...
			if (phone_temp == KH )
			{
				...
				if (begtyp[phnex_temp] != 1)
				{
					tartemp = 53;	   /* Stronger asp before +back */
				}
			}
			else
			{
	...
	else if (pDphsettar->par_type IS_PARALLEL_FORM_AMP)
	{
		...
		if ((pDphsettar->np != &PTILT) && ((tartemp = ptram[phone_temp]) > 0))
		{
			...
			{
			case 4:
				/* This error so bad I should fall on my sword */
				/*	This is a fatal errror*/
				/*exit(1);*/
				dt_abort;

				break;
			case 5: /* Added this case for begtyp 6 which was reaching past the table and grabbing data
					from the wrong phoneme since -1 for c arrays the 1 type is really a back*/
				begtypnex = 1;
				break;
			}
			/* switch */
...
static short special_coartic (PDPH_T pDph_t, short nfon, short diphpos)
{
	...
	{
		/* removed a !QQ condition below */
		if (((featb[foncur] & FVOWEL) IS_PLUS)) 
		 {
		 }
				   ...
				   short struccur, short feanex)
{
	...
	if (pDphsettar->par_type IS_FORM_FREQ)
	{
		...
		else
		{
			...
			/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
			/* german DECtalk, special transitions into 'L'            */
			/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
			//if (pDphsettar->phcur == L)
			//{
			//	pDphsettar->durtran = NF15MS;
//
//			} Questioning rule eab 4/6/98
			/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
			/* fast transition into plosives                   */
			/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
			if ((feacur & FPLOSV) IS_PLUS)
			{
				pDphsettar->durtran = NF15MS;
	...
	else if (pDphsettar->par_type IS_FORM_BW)
	{
		...
		if ((fealas & FNASAL) IS_PLUS)
		{
			...
			if ((pDphsettar->np == &PB2) && (begtyp[pDphsettar->phcur] != 1))
			{
				   				...
				   				short feanex, short strucnex)
{
	...
	if (pDphsettar->par_type IS_FORM_FREQ)
	{
		...
		else
		{
			...
			if (pDphsettar->np != &PF1)
			{
				if ((pDphsettar->phcur >= AH) && (pDphsettar->phcur <= UEH))
				{
					if ((pDphsettar->phonex == RR) || (pDphsettar->phonex == R))
					{
			   ...
			   short struccur, short pholas, short struclas)
{
	...
	if ((feacur & FBURST) IS_PLUS)
	{
			...
			((feacur & FPLOSV) IS_PLUS))
		{
			...
			{						   /* Was >= */
				/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
				/* change minumum burst duration in german DECtalk to 1     */
				/* bdur = 0;                    */
				/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
				bdur = 1;
			}
	...
	{
		/* EAB		3/2/98		At least temporarily reduce aspiration for noe it's too hot*/
		PAP.pspesh = 57-10;			   /* Amplitude of aspiration in dB     */
		if (begtyp[pDphsettar->phonex] != 1)
		{
			PAP.pspesh = 61-10;		   /* Stronger asp before +back */
		}
