 ...
 * 0016 EAB		7/8/98			added tcumdur stuff
 * 018  EAB		7/14/98 	Continued tuning this time I really changed tilt
 * 019  EAB		7/13/98		Found redundant tcumdur code that was not needed. It's calculated
 *							in phinton1/2 now.BATS 713 tcumdur addition removed
 * 020  EAB		7/20/98 	opied from  p_sp_tim.c after merge with _LA
 * 021 eab		7/22/98		Modified duration rules to support all levels of reduction on a monosylable
							and tuned up the syllable timed reference. 
022 	EAB 10/16/98   Tuned the Spanish
023     EAB 10/27/98   EAB In short comma phrases reduced the pause duration
024		EAB 10/29/98   Fixed duration of vowel following another vowel Juan often lost perception of scond vowel
						also fixed duration of intervolcalic n		
025 	EAB 10/29/98 	Further refinement of rules with Juan
026		EAB 11/12/98    BATS 816 see notes with bats for details
027		EAB  02/18/99	Combine like consonants across a word boundary
...
/* MAIN LOOP, for each output phoneme */
	...
	for (nphon = 0; nphon < pDph_t->nallotot; nphon++)
	{
		...
		if (strucstresscur IS_MINUS)
		{
				...
				//10/29/98 EAB Since adjecent vowel was stressed let's not get so virgourous
			{
				...
				//10/29/98 EAB Since adjecent vowel was stressed let's not get so virgourous
				if((featb[pDph_t->allophons[nphon-1]] & FVOWEL) IS_MINUS)
					durmin >>= 1;
				}
			}
		...
		if (feasyllabiccur IS_PLUS)
		{
			if((struccur & FTYPESYL) == FMONOSYL)
			{
				...
				if (strucstresscur IS_MINUS)
				{
					...
					// of anything by las......
					if(pDph_t->allophons[nphon-1] == E_L && pDph_t->allophons[nphon] == E_A
									&& pDph_t->allophons[nphon +1] != E_S)
					{
	...
	/* 2/9/88 EAB, 
		...
		*/
		...
		//NEWJUAN
			...
			if((featb[pDph_t->allophons[nphon+1]] & WBOUND) IS_PLUS)
			{
				...
				if(pDph_t->allophons[nphon+2] == pDph_t->allophons[nphon+1])
				{
					...
					if(/*featb[pDph_t->allophons[nphon+1]] & FSTOP
					   eab stop not marked in spanish I think it should be
						||*/ featb[pDph_t->allophons[nphon+1]] & FBURST)
						deldur=-NF30MS;
		...
		if(pDph_t->number_words == 1
			&& ((feacur & FVOWEL) IS_PLUS)) //lengthen  vowels in single word phrases
		{
			pDphsettar->durxx += NF40MS;
		}
