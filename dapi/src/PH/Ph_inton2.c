 ...
 * 0038 EAB		9/29/98			Found some hidden problems ewhere values
   ...
   0040 EAB 10/22/98 Bats 776- Some arrays were too short for max pointer movement
    ...
    0050 EAB    4/17/99		TUNign changes for 4.6
	0051 CAB	08/11/00	Removed WINprintf for windows ce.
...
/*%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
short f0_mstress_level[] = {1, 11, 31, 21, 41+30, 1, 130, 1};
...
//	pDph_t->commacnt =0; //BATS709
	...
	/* EAB Part of BATS 600 nphon->n_last_syl_nuc*/
		for(n_last_syl_nuc=pDph_t->nallotot;n_last_syl_nuc > 1;n_last_syl_nuc--)
			{
			if ((featb[pDph_t->allophons[n_last_syl_nuc]] & FSYLL) IS_PLUS)
				break;
			}
#endif
...
#ifdef ENGLISH
	    if ((fealas & FVOICD) IS_MINUS && pholas != SIL) {
		if ((feacur & FSON1) IS_PLUS) {
		    pDph_t->alloopenq[nphon] = 70;		/* Spread glottis */
		    if ((pholas == F) || (pholas == TH) || (pholas == S)
		     || (pholas == SH) || (pholas == CH)) {
...
/* eab 2/21/97 EAB remove emphasisflag and associated stuff pulled see note in phsort.c */
					...
					if(pDphsettar->nrises_sofar == 0)
						make_f0_command ( phTTS,GLIDE, 22, 100, -6,pDph_t->allodurs[nphon], &cumdur, nphon);
...
/*			  i.e. readjust timing so get fall-rise, not r-f-r */
 ...
 to move all of these kinds of things to the speaker def files. 2/26/97 */
					...
					else
					{
						...
						delayf0 -= NF20MS;
#if defined GERMAN || defined ENGLISH
						//BATS 709
			...
			if ((feacur & FSYLL) IS_PLUS)
			{
				...
				the phoneme following the syllable nucleus is voiced BATS 796. */
							if(featb[phonex] & FVOICD)
							{
								...
								{
									make_f0_command ( phTTS,GLOTAL, 5, targf0,  (pDph_t->allodurs[nphon]), 20, &cumdur, nphon);
								}
							}
							else
							{
...
#else
							make_f0_command ( phTTS, STEP,6, F0_CGesture2, delayf0, 20, &cumdur, nphon);
#endif
							pDph_t->commacnt++;
						}
