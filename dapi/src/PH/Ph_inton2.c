 ...
 *                                      Fixes BAT 346
 * 0018 EAB		04./18/97		Completed merge with 5.0 and French plus fixed a problem where somehow and extra line of code
 ...
 *  0047 EAB 	2/3/99			Added change from NWSNOAA->NWS_US for multi language NOA
 ...
 *	0049 EAb	6/7/99			BATS 897
 *  0050 EAB    4/17/99 		Tuning changes for 4.6
 *  0051 EAB	4/18/99 		Fixed ifdef for UK for N_last_sylL...
 *	0050 CAB	03/15/00		WINprintf not supported under Windows CE
 ...
 *  059 EAB		3/2/01  EAB Changed Fsonor back to FVOICD for rule 6 as silence has always
   ...
   060 EAB 3/13/01 Tuning of intonation with Winfried.
   061 EAB 3/28/01 EAB Fix bats 952 comma intonation
 *
 */
/* #define MSDBG4 */
...
#ifdef NWS_US
extern short us_f0_stress_level[];			/* F0 rise as f(stress-level)  	 */
#endif
...
#ifdef FRENCH
short F_Nb_Syll(PDPH_T pDph_t, short nphon) /* Number of vowels in the word from nphon */
{
  short NbS, i;
...
short f0_mphrase_position[] = {   91,  83, 76, 69,  63, 59, 55, 51, 49 };
/* Was: 210,  90, 40, 20 */
...
short f0_mstress_level[] = {0, 31, 11, 71, 0, 0, 0, 1};
		...
		if ((pDph_t->f0mode == NORMAL) || (pDph_t->f0mode == HAT_F0_SIZES_SPECIFIED))
		{
			...
			/* eab 1/19/98 previous target was upglide so now we
										want to do a downglide */
				{
					...
					make_f0_command ( phTTS,STEP, 20,targf0 , -6,pDph_t->allodurs[nphon], &cumdur, nphon);
					make_f0_command ( phTTS,STEP, 21, (short)(pDph_t->prevtargf0>>3), pDph_t->allodurs[nphon],pDph_t->allodurs[nphon+1], &cumdur, nphon); // NAL warning removal
					goto skiprules;
				}
...
//#endif
				...
				{
					targf0 = us_f0_stress_level[stresscur]-200;
				}
				else
				{
				...
				if (pDph_t->clausenumber == 0  && pDphsettar->lastbound==1) /*one == comma*/
					targf0 = targf0-(targf0>>2); // NAL warning removal
...
/* eab 2/21/97 EAB remove emphasisflag and associated stuff pulled see note in phsort.c */
	...
	//					make_f0_command ( phTTS,IMPULSE, 2, targf0, delayf0,20, &cumdur, nphon);
					...
					impulse_width = 0;
					nphonx = nphon;
					vowel_portion = pDph_t->allodurs[nphon];
					if(nextphrbou != nextwrdbou)
					{
						delayf0 = vowel_portion;
						if(nextwrdbou == nextsylbou) /* non-phrase final word, with stress on the final syllable */
						{
							...
							if((pDph_t->allofeats[nphonx] & FWBEND) IS_PLUS)
							{
								...
								impulse_width -= (pDph_t->allodurs[nphonx-1]);
							}else{
								impulse_width += (pDph_t->allodurs[nphonx]>>1) + (pDph_t->allodurs[nphonx]>>3); /* Add some of the final nucleus is */
							}
						}
					}else if(nextsylbou != nextwrdbou){
						impulse_width += pDph_t->allodurs[nphon];
						delayf0 = 0;
					}else{
					...
					if(impulse_width > 0)
					{
						make_f0_command (phTTS, IMPULSE, 26, targf0, delayf0, impulse_width, &cumdur, nphon);
						...
						//NOW A NEGATIVE PULSE
							...
							delayf0 = delayf0 + impulse_width; 

							impulse_width = 10;
			if(pDph_t->number_words > 2)
			{
						make_f0_command ( phTTS,IMPULSE, 29, targf0, delayf0, impulse_width, &cumdur, nphon);
			}
#ifdef GERMAN 
...
#ifndef FRENCH
						if (0)//keep this out for now((struccur & FBOUNDARY) == FQUENEXT)
						{
			...
			if ((feacur & FSYLL) IS_PLUS)
			{
				...
				{					   /* or 1-str nofall */
							...
							|| (phone_feature( pDph_t,phonex) & FCONSON) IS_PLUS))
					{
							...
							|| ((struccur & FBOUNDARY) == FEXCLNEXT))
						{
							...
							test_targf0 = targf0;	/* RSM created this variable to reduce final fall in GERMAN */
							impulse_width = 20;		/* RSM */
...
#ifdef GERMAN
							test_targf0 = targf0>>2;	/* RSM */
							impulse_width = 10;			/* RSM */
							if(pDph_t->number_words == 1)
							{
				...
				the phoneme following the syllable nucleus is voiced BATS 796. */
							if(phone_feature( pDph_t,phonex) & FVOICD)
							{
								...
								{
									make_f0_command ( phTTS,IMPULSE, 5, test_targf0,  (pDph_t->allodurs[nphon]), impulse_width, &cumdur, nphon);
								}
								else
								{
									delayf0 = (pDph_t->allodurs[nphon]>>1) + (pDph_t->allodurs[nphon]>>2);
									make_f0_command ( phTTS,IMPULSE, 5, test_targf0,  delayf0, impulse_width, &cumdur, nphon);
								}

							}
							else
							{
								make_f0_command ( phTTS,IMPULSE, 5, test_targf0, (short)(pDph_t->allodurs[nphon]>>1), impulse_width, &cumdur, nphon); // NAL warning removal
							}
#endif //ndef FRENCH
			...
			pDph_t->commacnt++;
				...
				else if (((struccur & FBOUNDARY) == FPERNEXT))
				{
					...
					*/
					...
					test_targf0 = targf0; /* RSM variable use to reduce the final fall in GERMAN */
					impulse_width = 20;	/* RSM */
					/* Sent.-final unstressed vowel followed by a period */
					/* eab 4/13/98 comment is wrong this is stressed vowel so review code in detail 
					when time permits*/
#ifdef GERMAN
					test_targf0 = targf0>>2;
					impulse_width = 10;
					if(pDph_t->number_words == 1)
					{
					...
					if(phone_feature( pDph_t,phonex) & FVOICD)
					{
						...
						if(nphon+2 <= pDph_t->nallotot && (phone_feature( pDph_t,pDph_t->allophons[nphon+2]) & FVOICD))
							/* nucleus has two voiced phonemes following it so delay even more*/
						{
							make_f0_command ( phTTS,IMPULSE, 6, test_targf0,  (pDph_t->allodurs[nphon]), impulse_width, &cumdur, nphon);
						}
						else	/* RSM */
						{
							delayf0 = (pDph_t->allodurs[nphon]>>1) + (pDph_t->allodurs[nphon]>>2);
							make_f0_command ( phTTS,IMPULSE, 6, test_targf0,  delayf0, impulse_width, &cumdur, nphon);
						}
					}
					else
					{
						make_f0_command ( phTTS,IMPULSE, 6, test_targf0, (short)(pDph_t->allodurs[nphon]>>1)+3, impulse_width, &cumdur, nphon); // NAL warning removal
					}
