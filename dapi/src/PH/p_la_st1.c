 ...
 *  007 EAB		05/13/1998	Added slowtalk code BATS660
 *	008 EAB		05/20/1998  BATS681  Remove unneeded code--debugforward && debugbackwards
 *	009 GL		05/27/1998  BATS685  Remove unwanted debug code. fix the compiling problem
 *  010 EAB		07/08/1998	Changes to solve addition of new Phedit style of phrom
 *  011 EAB 	07/14/1998 	Continued tuning for new code
 *  012 EAB		07/20/1998 	Copied from p_sp_st1.c after merge with new Latin file structure
 *  013 EAB		07/23/1998	The tilt code is bad in that it is all else if but the ordering
 ...
 *  014 EAB 	10/12/1998 	Tuned voicebar to a lower value to avoid problems
 *  015 EAB 	10/23/1998  Modified aspiration per info Juan found 
 *    						in University of Chicago Spanish Dic.
 *  016 EAB		03/26/1999	EAB BATS 878 Fixed the burst-plosive and plosive-plosive problem 
 *  017 EAB		06/07/1999	Modify to support Nasal value definitions
 *  018 MGS		04/13/2000	Changes for integrated phoneme set 
 *  019 EAB		05/25/2000 	Can't use ifdef OUT
 *  020 EAB		06/28/2000	Unified Phoneme Set Changes
 *  021 CAB		10/16/2000	Changed copyright info and edited mistakes in comments
 *	022	MGS		12/13/2000	Further optimization on gettar
 *	023	MGS		01/18/2001	Removed arg1 arg2 and arg3 from calculations
 */
...
short la_gettar (LPTTS_HANDLE_T phTTS, int nphone_temp)
{
	...
	if( PFLA != pDph_t->last_lang)
	{
		pDph_t->last_lang = PFLA;
			...
			if ((tartemp = pDph_t->p_tar[(phnex_temp & PVALUE) + pphotr]) == -1)
			{
				...
				if ((tartemp = pDph_t->p_tar[(get_phone (pDph_t, nphone_temp + 2) & PVALUE) + pphotr]) == -1)
				{
					...
					if ((tartemp = pDph_t->p_tar[(phlas_temp & PVALUE) + pphotr]) < -1)
					{
						...
						{
							/* eab 7/8/98 ++ is going the 
							wrong dirextion in the array*/
							tartemp--;
						}
 ...
 */
	 ...
	 */
	temp += la_spec_coart(pDph_t,foncur, fonlas);
	temp += la_spec_coart(pDph_t,foncur, fonnex);

	return (temp);
}

/*
 *      Function Name: la_spec_coart()      
 *
...
plosive-nasal simply doesn't release eab 12/27/99 */
		...
		{
		if (la_place[pDphsettar->phcur & PVALUE] == la_place[pDphsettar->phonex & PVALUE]) 
			{	/* Was >= */
				bdur = 0;
			}
	    }
