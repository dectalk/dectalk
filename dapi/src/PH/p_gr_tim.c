 ...
 *								(high for German) was able to glitch the filters when ramping too rapidly from silence
								BATS 667
 * 0034 EAB		6/9/98			Fixed BATS 709 This problem  exists in all languages. When a plosive is folloed by another plosive
								the second one is slower......

 * 0035 EAB		7/2/98			BATS 709 Per Oliver this code allows for vowels before double consonants to have a faster duration
 * 0036 EAB		7/8/98			BATS 709-MISC
 * 0037 EAB		7/13/98			Found redundant tcumdur code that was not needed. It's calculated
 *								in phinton1/2 now.BATS 713 tcumdur addition removed
 * 0038 EAB		8/5/98			BATS 734
 * 0039 EAB		2/3/99          Did some more work on syllable timed rythm
...
void phtiming (LPTTS_HANDLE_T phTTS)
{
...
#ifdef GERMAN // bats 709 back in for now as a result of finer tuning of other arreas  7/8/98
 /* For German this is at least way to strong and may not be true at all*/		
		...
		{
			deldur = 0; /*eab don't lengthen ich i.e unvoiced fricatives.
			BATS 734*/
		...
		//kills everything
		/* Rule 3: Shortening of non-phrase-final 
		syllabics (pp counted as phrase only at slow speaking rates) */
		...
		if (feasyllabiccur IS_PLUS)
		{
			if (/*((strucstresscur & FSTRESS_1) IS_MINUS)
				&& */((struccur & FTYPESYL) == FMONOSYL) /* NEED TO ADD NOT NOUN*/)
			{
					 ...
					 && ( strucboucur < FWBNEXT))
			{
				...
				{
					/* Other nonfinal syllables shortened by 0.85 eab 4/18*/
...
/* Rule 7: Shortening of unstressed segs */
				...
				&& ((pDphsettar->feanex & FOBST) IS_PLUS))
		{
				...
				&& ((pDphsettar->strucnex & (FSTRESS | FWINITC)) IS_MINUS))
			{
				...
				if ( (((featb[pDph_t->allophons[nphon + 2]] & FCONSON) IS_PLUS)
					&& ((featb[pDph_t->allophons[nphon + 2]] & FSONOR) IS_PLUS))
...
//				hurt		influence
	...
	languages */
		if (((featb[pDph_t->allophons[nphon]] & FPLOSV) IS_PLUS) && ((featb[pDph_t->allophons[nphon-1]] & FPLOSV) IS_PLUS))
		{
			durmin += NF40MS;
		}
		/*EAB		7/8/98		BATS 709*/
