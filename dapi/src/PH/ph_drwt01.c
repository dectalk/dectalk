 ...
 * 054  EAB		2/3/99	Added change from NWSNOAA->NWS_US for multi language NOA
 * 055  EAB		2/23/99 Added in copyright plus new gov wording plus removed incorrect
						time adjustment(found Bill Hallhan had added comp in
						vtm
 * 056  EAB		5/3/99  Fixed BATS 887
 * 057  EAB		5/3/99  Flattened out intonation for LOW_COST_VERSION
 * 
 #ifdef GERMAN
 @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
 ...
 *	0036  MGS		12/17/1999	Fix for SAPI
 */
...
/*        Set next f0 segmental incremental target */
	...
	f0seg = f0segtars[pDphsettar->phocur];
...
/*    Approximate sine by one minus t squared				*/
	cosine = (temptime * temptime) - DT_ONE;
	if (time != temptime)    return(cosine);
	else    return(-cosine);
}
#endif
#else /*Non-NWS_US & Non-ENGLISH_UK next*/
