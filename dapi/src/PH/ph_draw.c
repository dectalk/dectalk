...
* 0054 EAB 3/13/01 Add allophonic variant of R phoneme where it becomes less voiced and aspirated if followed
	by an unvoiced...
2/28/01 eab fix burst for german ts phonmeme
*/
...
#endif 	// THETRILLISNOTGONEBUTITSGOING
#endif	// SPANISH
...
/* Spdef par FT -> 0 < f0_dep_tilt < 100, temptilt = 1/40 (max) of 1400-f0 */
	...
	*parp += temptilt;
	//EAB		7/13/98			BATS 711 
#ifdef AD_BASE	//AD_BASE defined in dectalkf.h for every language
	*parp += pDph_t->spdeftltoff;
#elif FRENCH	// not used
	*parp += (spdeftltoff - 3); /*from fphdraw.c*/
#else
	/* Use -6 to allow higher tilts eab TILT IS A MESSS FULL OF UNKNOWN FUDGE FACTORS 
	   LIKE THIS -6 AND IN GENERAL TOO STRONG, TOM HAS FOUND THE TILT FILTER 
       TO HAVE TERRIBLE CHARACTERISTICS SO I'M DOING A QUICK FIX 
       FOR NOW WHICH WILL BE MUCH REFINED WHEN WE CAN IMPLEMENT THE NEW FILTER 
	...
	*parp += pDph_t->spdeftltoff - 6;
	...
	if(*parp > 18)	// end of this if ???????????????? for the different languages
	{
#ifdef DEBUGPHT
#if defined (WIN32) && defined (PRINTFDEBUG)
	WINprintf("til3=%d \n",*parp);
...
#if defined (ENGLISH_US) || defined (SPANISH) || defined (GERMAN) || defined (FRENCH)
...
/* eab 6/25/98 cleanup in AD and RND Old code that helps nothing */
...
#ifdef SPANISH_SP
	if (pDph_t->allophons[ pDph_t->nphone] == SPP_R)
#endif
#ifdef SPANISH_LA
...
#ifdef SPANISH_LA
	else if (pDph_t->allophons[ pDph_t->nphone] == LAP_RR)
#endif
	{
		// NEW_SPANISH  EAB		7/13/98			BATS 711
...
#ifdef ENGLISH_US //for hebrew phonemes
	...
	//	h_modulation (phTTS,pDph_t->param[A2].outp, N5PRCNT, FALSE);
	//	h_modulation (phTTS,pDph_t->param[A3].outp, N5PRCNT, TRUE);
	//	h_modulation (phTTS,pDph_t->param[A4].outp, N10PRCNT, FALSE);
	//	h_modulation (phTTS,pDph_t->param[A5].outp, N10PRCNT, FALSE);
	...
	else if( pDphsettar->phcur == GRP_TJ)
	{
		pDphsettar->delay_cnt++;
		if(pDphsettar->delay_cnt <5)
		{
...
/* DEBUG Code  verifies variables that should never hit 
	...
	if((phone_feature( pDph_t, pDph_t->allophons[ pDph_t->nphone]) & FNASAL) IS_PLUS)
	{
		if(pDph_t->malfem == MALE)
		{
			/* eab 4/23/98 If phoneme is less than M then it's a nasalized french vowel*/
			if(pDph_t->allophons[ pDph_t->nphone] <	GRP_M)
			{
	...
	if(pDph_t->allophons[ pDph_t->nphone ] == GRP_Z)
	{
		if (pDph_t->malfem == MALE)
		{
...
#ifdef SPANISH_SP
	if(pDph_t->allophons[ pDph_t->nphone] == SPP_RR)
#endif

#ifdef SPANISH_LA
	if(pDph_t->allophons[ pDph_t->nphone] == LAP_RR)
#endif
	{
...
#ifdef SPANISH_SP
	if(pDph_t->allophons[ pDph_t->nphone ] == SPP_Z)
#endif

#ifdef SPANISH_LA
	if(pDph_t->allophons[ pDph_t->nphone ] == LAP_Z)
#endif
	{
		if (pDph_t->malfem == MALE)
		{
	...
	if(pDph_t->allophons[ pDph_t->nphone] == LAP_BH )
#endif
	{
		if (pDph_t->malfem == MALE)
		{
