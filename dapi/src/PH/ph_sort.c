...
#endif
#if defined ENGLISH || defined SPANISH_OUT
...
#endif
#if defined SPANISH_OUT || defined GERMAN 
...
#ifdef BOOKSAYSDIFFERENT //from previous input perhaps a castillion/latin difference or
			// maybe anna was wrng/anglicanixzed
		/* 12/27/96 EAB insert a glotal stop bewteen s#s V#s and s#v and 
				...
				else if(featb[pDph_t->symbols[n-1]] & FSONOR)
			{
					...
					|| 	pDph_t->symbols[ntmp] == E_S || pDph_t->symbols[ntmp] == E_TH)
				{
					...
					/* BATS 677 Found another spot it was doing it wrong
					EAB 5/18/98 
					EAB Found a behavior problem because N+1 can be end
...
a stress*/
		...
		eab In spanish we need to combine to like vowels together with 
		lengthen dur unlike english where a glotal stop effect is ok 
		Opps we need to verify that they're real phonemes and not control phones
		eab sep 4,1998 eab this code stinks do it in ph_timing*/

		if ( (pDph_t->symbols[n-1] < 100) &&
			(pDph_t->symbols[n] == WBOUND || pDph_t->symbols[n] == 108 ))
		{
			if (pDph_t->symbols[n - 1] == pDph_t->symbols[n + 1])
			{
				delete_symbol (phTTS, n + 1);
