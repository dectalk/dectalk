 ...
 *							the wrong place. This had potetential of causing other nasty problems. 
 ...
 * 0034 EAB     5/26/99		BATS 894 Inter vocalic t are no longer flapped for better articualtion but
...
/* Rule turned on again with slightly less reduction I think this will be OK*/
		...
		if (pDphsettar->phonex_timing == T
			&& ((feacur & FVOWEL) IS_PLUS)
			&&((featb[pDph_t->allophons[nphon+2]] & FVOWEL) IS_PLUS)
		...
		/* RULE 24: Suggested by EVAN "ing's" were tooo long analysis showed
		that the vowel needed reduction */
		if (pDph_t->allophons[nphon+1] == NX)
		{
			arg1 = prcnt;
			arg2 = N60PRCNT;
			prcnt = mlsh1 (arg1, arg2);
