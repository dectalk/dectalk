...
$
 * Revision 1.1.2.2  1995/07/05  20:04:08  Krishna_Mangipudi
 * 	Moved from ./src/synth/fvtm
 * 	[1995/07/05  20:02:45  Krishna_Mangipudi]
 *
 * Revision 1.2.2.2  1994/11/07  22:55:35  Krishna_Mangipudi
 * 	Initial Check-in
 * 	[1994/11/07  20:37:13  Krishna_Mangipudi]
 * 
...
#define DESIGN_TWO_POLE_FILTER_CF45( A1, A2, UnityGainAtDC, Frequency, Bandwidth ) \
  ...
  { \
    Radius = RadiusTable[(int)( 0.125 * BW )]; \
    A1 = Radius * TwoCosineTable[(int)( 0.125 * Freq )]; \
    A2 = - Radius * Radius; \
  } \
  UnityGainAtDC = 1.0 - A1 - A2;

/**********************************************************************/
/*                                                                    */
/*  Macro: DESIGN_TWO_POLE_FILTER_CF123                               */
/*  Author: Bill Hallahan                                             */
/*  Date: September 5, 1993                                           */
/*                                                                    */
/*  If the filter response extends too near to Fs/2 then this macro   */
...
/*  If the formant frequency move over Fs/2, this macro will cause    */
/*  the filter to be effectively removed (over a short filter decay   */
/*  time) without causing an abrupt gain change.                      */
/*                                                                    */
/**********************************************************************/

#define DESIGN_TWO_POLE_FILTER_CF123( A1, A2, UnityGainAtDC, Frequency, Bandwidth ) \
  BW = InverseSampleRateScale * Bandwidth; \
  Freq = InverseSampleRateScale * Frequency; \
  if ( Freq + 0.5 * BW >= 4500.0 ) \
  { \
...
/**********************************************************************/
/*                                                                    */
/*  Macro: DESIGN_TWO_POLE_FILTER_PF                                  */
/*  Author: Bill Hallahan                                             */
/*  Date: September 5, 1993                                           */
/*                                                                    */
/*  If the filter response extends too near to Fs/2 then this macro   */
...
#define DESIGN_TWO_POLE_FILTER_PF( A1, A2, UnityGainAtDC, Frequency, Bandwidth ) \
  BW = InverseSampleRateScale * Bandwidth; \
  Freq = InverseSampleRateScale * Frequency; \
  if ( Freq + 0.5 * BW >= 4500.0 ) \
  { \
