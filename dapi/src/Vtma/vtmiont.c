...
/*							(bats546)                                 */
/* xx	20nov98		GL		BATS#828 use VTM_DEBUG to replace _DEBUG
/*																	  */
/**********************************************************************/
/**********************************************************************/
/*  Include Files.                                                    */
...
/* For FVTM iwave is not necessary to be a thread specific ,can be made local here :MVP */
...
#ifdef VTM_DEBUG
  {
	...
	case STATE_OUTPUT_AUDIO:
	  ...
	  // use the below ifdef insteadof the above to use this feature.
//#ifdef OLEDECTALK // tek 22aug97 try to send a dest bookmark..
		 ...
		 )
	  {
		  ...
		  if (!PA_SetBookmark(phTTS->pAudioHandle,(DWORD)pMarkData))
		  {
			...
			{
				char szTemp[256]="";
				sprintf(szTemp,"AudioDest BookMark %lx set at %lu\n",
		  ...
		  else 
		  {
			...
			{
				char szTemp[256]="";
				sprintf(szTemp,"AudioDest BookMark %lx failed at %lu, using sync_pipe\n",
					pMarkData,timeGetTime());
				OutputDebugString(szTemp);
			}
#endif //VTM_DEBUG
...
#ifndef OLEDECTALK // remove this ifdef for bats546!
	if ( dwPhoneme != pKsd_t->dwLastPhoneme )
	{
...
void SendVisualNotification(LPTTS_HANDLE_T phTTS, DWORD dwPhoneme, DWORD dwDuration)
{
	...
	if (pvdPacket) // just punt if the malloc failed
	{
		...
		{
			char szTemp[256]="";
			sprintf(szTemp,"SendVisualNotification: pho:%02lx (%08lx) [%c], dur:%04ld, samp:%lxx TFC:%ld. at %lu\n",
