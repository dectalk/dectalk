...
*  16   11feb00 	EAB		Integrated hlsyn
*  17	12jun00		NAL		Warning removal
*  18	14jul00		MGS		Sapi 5 additions
*  19	20jul00		MGS		More Sapi 5 stuff
*  20	26jul00		MGS		Sapi 5 visuals
*  21   28jul00		EAB		Added ouput of phone+1 for SAPI
*  22	22aug00		MGS		BATS #935 Fixed 2 concurreny problems
*  23	16oct00		CAB		Fixed copyright and formatted comment section.
*  24	02feb01		MGS		Merged in base changes needed for ACCESS32 merge
*  25   01mar01		CAB		Updated copyright info
*  26   27mar01		MGS		Visual marks for Tru64 (not working correctly right now)
*  27	27mar01		MGS		Remove visual marks for CE	
*  28	28mar01		MGS		Found that the save of the last phoneme was masked for 
...
int temp=0;	
  ...
  llspeaker.GH = 72;		/* gain of aspiration noise source */
  llspeaker.GF = 78;		/* gain of frication noise source */
	  ...
	  if (( ! pKsd_t->halting ) && ( ! phTTS->bMemoryReset ))
	    {
		  ...
		  //printf("al = %f \n",frame.al);
		   if(pVtm_t->parambuff[OUT_AV+1] >56)
			   frame.ag = 4.5f;
		   else if (pVtm_t->parambuff[OUT_AV+1] == 0)
			   frame.ag = 0.0f;
		   else
			   frame.ag = 2.0f;
		  frame.ab = 100.0f;
		  frame.an = pVtm_t->parambuff[OUT_FZ+1];
		  if(frame.an > 30)
			frame.al = 0;
		  frame.ue = 0.0f;
		  frame.f4 = 3500.0f;
...
//	  llframe.NOQ = pVtm_t->parambuff[OUT_OQ+1];
	  ...
	  llframe.NAB = pVtm_t->parambuff[OUT_AB+1];
...
//	printf("out_f1,  %d %f %f\n",pVtm_t->parambuff[OUT_F1+1],frame.al,frame.ag); 		  
	...
	pVtm_t->parambuff[OUT_FZ+1] =llframe.NFNZ;
	pVtm_t->parambuff[OUT_FNP+1] =llframe.NFNP;
	//printf(" fnp %d \n",llframe.NFNP);
	//printf(" fnz %d \n",llframe.NFNZ);
	if (llframe.NFNZ > 600)
	//	printf(" fnz %d \n",llframe.NFNZ);
  ...
  InitializeHLSynthesizer(&oldframe, &speaker, &oldstate, MALE);
	...
	case (SPC_type_index|SPC_subtype_volume):
		...
		if (pMarkData)
		{
		  pMarkData->qTimeStamp = dwSampleNumber;
		  // glue together the two halves of the index value..
		  pMarkData->dwMarkValue  =  (LPARAM)( (dwSyncParams[1]<<16) | (dwSyncParams[2]&0xFFFF));
		  pMarkData->dwMarkType = control;
#ifdef VTM_DEBUG  //tek15aug97
	...
	case STATE_OUTPUT_AUDIO:
	  ...
	  if (  (control ==  (SPC_type_index	| SPC_subtype_bookmark))
		  ...
		  ||(control ==  (SPC_type_index	| SPC_subtype_sentence))
		  ||(control ==  (SPC_type_index	| SPC_subtype_stop))
		  ||(control ==  (SPC_type_index	| SPC_subtype_volume))
		 )
	  {
	...
	case STATE_OUTPUT_SAPI5:
		...
		if (!pKsd_t->halting)
		{
			if (dwSampleNumber!=0 || control == SPC_type_index)
			{
				if  (control ==  (SPC_type_index	| SPC_subtype_bookmark))
				{
					Report_TTS_Status(phTTS,phTTS->uiID_Bookmark_Message,dwSampleNumber,(LPARAM)dwSyncParams[1]);
					free((void *)(dwSampleNumber));
				}
				else if  (control ==  (SPC_type_index	| SPC_subtype_wordpos))
				{
					SendEventToSapi(phTTS,phTTS->uiID_Wordpos_Message,dwSampleNumber,(LPARAM)dwSyncParams[1]);
					free((void *)(dwSampleNumber));
				}
				...
				else if  (control ==  (SPC_type_index	| SPC_subtype_stop))
				{
					Report_TTS_Status(phTTS,phTTS->uiID_Stop_Message,dwSampleNumber,(LPARAM)dwSyncParams[1]);
					free((void *)(dwSampleNumber));
				}
				else if  (control ==  (SPC_type_index	| SPC_subtype_start))
				{
					SendEventToSapi(phTTS,phTTS->uiID_Sentence_Message,dwSampleNumber,(LPARAM)dwSyncParams[1]);
					free((void *)(dwSampleNumber));
				}
				else if  (control ==  (SPC_type_index	| SPC_subtype_volume))
				{
					SendEventToSapi(phTTS,(SPC_type_index | SPC_subtype_volume),dwSampleNumber,(LPARAM)dwSyncParams[1]);
					free((void *)(dwSampleNumber));
				}
				else
					Report_TTS_Status(phTTS,phTTS->uiID_Index_Message,(WPARAM)dwSyncParams[2],(LPARAM)dwSyncParams[1]);
