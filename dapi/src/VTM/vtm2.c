...
*
 *	59 MFG	02/27/2003	Merged in DECtalk version 5.00 changes
...
//	FNPinHZ= variabpars[OUT_FNP];
  // 4/15/00 New code to control the nasal pole zero pair based on the area of opening
  /* per the new stuff by Ken Stephens, Since the equaiotns are too diificult to type
  see notes staring pg 122 in enginering notebook Force1 We will do this by sucessive approx
  to deterimne how detailed the table needs to be
  the area table is refernece by the area in cm2 tiems 10 *2 where the even numbers are the
  FZ and the odd nymbers are the corresponding FNP's and by an index from 1 to 5
  meaning from front to back*/
  ...
  pVtm_t->rnppa = (FZinHZ-290)>>1;
    if(pVtm_t->rnppa)
	{
			pVtm_t->rnppa = pVtm_t->rnppa - (pVtm_t->rnppa >>2);
  ...
  d2pole_pf( pVtm_t,&pVtm_t->rnppb, &pVtm_t->rnppc, fnp, bnp, 0 );
...
/*    Tilt spectrum of voicing source down by soft low-pass filtering, amount
	...
	temp =  frac1mul((abreth), noiseb  );
	voice += frac4mul( pVtm_t->avlind, temp );
