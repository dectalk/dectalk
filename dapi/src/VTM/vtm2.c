...
void setabc(int f, int bw, short *acoef,short *bcoef,short *ccoef,short gain);
...
//23  BATS 731 EAB 7/31/98 Init glotnoise
  S16 glotnoise=0; /* eab new way of injecting noise pitch synchronously*/
  S16 noiseac;
#endif
    ...
    { 
      ...
      if ( pVtm_t->nper > ( pVtm_t->T0 - pVtm_t->nopen ))
      {
		...
		if(pVtm_t->avlind > 40 && glotnoise == 0)
		 {
			//glotnoise = frac1mul( pVtm_t->aturb1, 16000 );
		 }
#endif
      }
      else
...
/*	      For reasons that I don't understand, amplitude compensation
	...
	setzeroabc( FZinHZ/*(500+((FZinHZ-263)*6))*/,120, FZinHZ-100/*400*/, &pVtm_t->rnza, &pVtm_t->rnzb, &pVtm_t->rnzc);
	  }
