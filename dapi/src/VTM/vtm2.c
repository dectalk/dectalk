...
/*  16. 26-MAY-97 EAB Fixed "poping problem see note in code		  
	...
	23  BATS 731 EAB 7/31/98 Glotnoise needs to be initalized-doesn't self reset properly otherwise suspect it came up
		negative.
	...
	29	tek 28oct98 change getmax to be switched on the fly; add COMPRESSION
	...
	41 3/13/00 New noise genrator and breathiness calculations needed some adjust(ph 888 was 
		always too strong---refine later
	42 4/11/00   EAB  Nasal filter out by error- how could tuner run??
	43 4/1700    EAB  Found problems with nasals--fixed for 4.6 check in
...
#ifdef CLOSING_TIME
  S16 Tgain;	// eab 4/22/99 gain of wang's closing time filter
#endif
#ifdef NEW_NOISE
...
//   TILTDB = frac4mul(819,temp /*variabpars[OUT_TLT]*/)+pVtm_t->oldtilt;             

 TILTDB = (TILTDB + pVtm_t->oldtilt)>>1;
  if(TILTDB < 0)
	  TILTDB= 1;	  
  if(TILTDB >= 40)
	  TILTDB = 40;
  pVtm_t->oldtilt=TILTDB;
#endif
#ifdef NEW_NOISE
...
#ifdef not_with_cf123 //It scales
 ...
 */
	...
	if (temp < pVtm_t->Aturb)    
		temp = pVtm_t->Aturb-(pVtm_t->Aturb>>1);
...
/*                This results in boost of as much as 8 dB at f=100 fund comp */
...
#ifdef CLOSING_TIME
	// For firat pass use tiltdb as a driving parameter because
	// we don't have an closing time argument EAB 
	  pVtm_t->decay= decaytab[TILTDB];
	//pVtm_t->decay= 29648; //eab used by wang to force values
  ...
  default:
	...
	/**************************************************************/
#ifdef NEW_VTM
	//FNPinHZ = (500+(FZinHZ-263)*3);
	//FNPinHZ = (500+((FZinHZ-263)*6)); //eab idebtical to zero to check performance
	d2pole_cf123( pVtm_t,pKsd_t, &pVtm_t->rnpb, &pVtm_t->rnpc,FNPinHZ , 120 ,0 );
#endif
...
/*	      For reasons that I don't understand, amplitude compensation
    ...
    Notch_Filter(voice,pVtm_t->rnzd1,pVtm_t->rnzd2,pVtm_t->rnzd3,pVtm_t->rnzd4, pVtm_t->rnza, pVtm_t->rnzb, pVtm_t->rnzc,200); 
#else
#ifdef NEW_TEST
/*  Adjust memory variables to have proper levels for a given sudden
              change to F1inHZ.
...
/*	      For reasons that I don't understand, amplitude compensation
	...
	setzeroabc( FZinHZ,80, 93, &pVtm_t->rnza, &pVtm_t->rnzb, &pVtm_t->rnzc);
	  }
...
#else
#ifdef CLOSING_TIME
//	Tgain=Tgaintab[TILTDB]-20;
//	pVtm_t->one_minus_decay = frac4mul( pVtm_t->one_minus_decay, Tgain);
...
/*    Tilt spectrum of voicing source down by soft low-pass filtering, amount
    ...
    /*  output is rnpd1.                                              */
    /******************************************************************/
	//pVtm_t->rnpad = pVtm_t->rnpa - ((FZinHZ-200)<<1);
...
//	pVtm_t->iwave[ns] = pVtm_t->r2cd1; 
    ...
    out = about - out;
	 ...
	 if (pVtm_t->avlind == 0 && variabpars[OUT_PH] == 0 )
	 {
