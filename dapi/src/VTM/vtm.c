...
/*  16. 26-MAY-97 EAB Fixed "poping problem see note in code		  
	...
	24  EAB Modified code for Tom to use much less computes eab 10/12/98
		The ifdef's to use are LOWCOMPUTE, LOWER_YET and LOWEST in that order
	25  K2 needs to be adjusted for lowcompute as well,it was breaking voices 
		having nf >0. 
...
#ifdef NEW_NOISE
  S16 noiseb;	//eab 10/12/98 New noise variables
  S16 noisef;	
#endif
...
/*      pVtm_t->randomx = (pVtm_t->randomx * ranmul) + ranadd;*/
	...
	noise = noisef + frac1mul( 24576, pVtm_t->noalast);
	pVtm_t->noalast = noisef;
    ...
    for ( nsr4 = 0; nsr4 < 4; nsr4++ )
    {
	  ...
	  /* Reset tilt filter at glottal open time (WHY NOT AT CLOSE TIME?) */
		...
		/* BUG: Some kind of obscure overload if TILTDB==0,1 */
				TILTDB -=8; //EAB temp kludge until it's tuned				
				if (TILTDB < 2)    TILTDB = 2;
				if (TILTDB > 41)   TILTDB = 41;
				Ftilt = frac1mul(20000,tiltf[TILTDB]);  /* 0.6 (fold into table)*/
...
/*                This results in boost of as much as 8 dB at f=100 fund comp */
...
#else
	if ( pVtm_t->nopen > (95>>2) )
	{
	  pVtm_t->temp = (S32)pVtm_t->temp * (pVtm_t->nopen<<2);
	  pVtm_t->a = frac1mul( 10923, pVtm_t->temp );
	}
	else
	{
	  pVtm_t->temp = frac1mul( 10923, pVtm_t->temp );
	  pVtm_t->a = (S32)pVtm_t->temp * (pVtm_t->nopen<<2);
	}
#endif
	...
	voice=voice<<1; //eab very crude gain adjust for testing 10/12/98
...
void read_speaker_definition(LPTTS_HANDLE_T phTTS) 
{
  ...
  */
  ...
  avg = ((SP_CHIP *)spdeftochip)->azgain-2;     /*  21                  */
  if (avg<0) avg=0;
