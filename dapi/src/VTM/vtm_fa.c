...
#define  Q15_INVERSE_SCALE          (Q12_INVERSE_SCALE/8.0)
  /********************************************************************/
...
#define SWG_INIT_VALUE 0 //(12345)
  ...
  FLTPNT_T Noisef=SWG_INIT_VALUE;       /*  Flat spectrum noise */
  FLTPNT_T Noiseb=SWG_INIT_VALUE;       /*  Noiseb breathiness (less low freq )  */
  FLTPNT_T nolast=SWG_INIT_VALUE;       /*  memory  */
  FLTPNT_T noblast=SWG_INIT_VALUE;       /*  memory  */
  ...
  FLTPNT_T Frics=SWG_INIT_VALUE;      /*  Output traient pulse for plosives   */
  FLTPNT_T amp_imp=SWG_INIT_VALUE;
...
//FUTURE Fix the tilt filter for now just don't let it get too big.
  ...
  {
    p2_gain = vtmLocal.SpeakerFricationGain * dBtoLinear[A2inDB + 13];
    p3_gain = vtmLocal.SpeakerFricationGain * dBtoLinear[A3inDB + 10];
  ...
  it's a place holder EAB 5/27/98*/
	FNPinHZ=InverseSampleRateScale *variabpars[OUT_FNP];
#ifdef HLSYN
  ...
  for ( uiNs = 0; uiNs < vtmLocal.uiNumberOfSamplesPerFrame; uiNs++ )
  {
    ...
    vtmLocal.randomx = vtmLocal.randomx * RandomMultiplier + RandomAddend;
		...
		Noisef = 0.25 * (FLTPNT_T)vtmLocal.randomx; /*flat_spectrum*/
...
are diff
    ...
    if ( vtmLocal.uiVoicePeriodSampleNumber < vtmLocal.uiPositionToStartNoiseModulation )
      Noise = 0.5 * Noise;
	...
	Frics = vtmLocal.SpeakerFricationGain*Noise+amp_imp*50000*(uiNs==0);
...
{ 
	if(dF1hz > 0 || dB1hz > 0) //glottis closes
      ...
      if ( vtmLocal.uiVoicePeriodSampleNumber > (vtmLocal.T0 - vtmLocal.iNumOpenSamples ))
      {
      ...
      if ( vtmLocal.uiVoicePeriodSampleNumber == vtmLocal.T0 )
      {
        /**************************************************************/
        ...
        vtmLocal.uiPositionToStartNoiseModulation = 0;

        if ( vtmLocal.LaryngealSourceGain > 0.0 )
          vtmLocal.uiPositionToStartNoiseModulation =  vtmLocal.iNumOpenSamples ;
...
#else
		vtmLocal.iNumOpenSamples = (int)(vtmLocal.k1 * vtmLocal.T0 + (vtmLocal.k2/4.0));
		vtmLocal.iNumOpenSamples += ( TiltInDB );
#endif
       // 
...
#else
		 if ( vtmLocal.iNumOpenSamples < 10 )
        {
          vtmLocal.iNumOpenSamples = 10;
        }
        else
        {
          if ( vtmLocal.iNumOpenSamples > 263/4 )
          {
            vtmLocal.iNumOpenSamples = 263/4;
          }
        }
#endif
...
#else
	vtmLocal.b = 0.0625 * B0[vtmLocal.iNumOpenSamples-9];
#endif
  ...
  DESIGN_TWO_POLE_FILTER( vtmLocal.NasalResonator_a1,
                          vtmLocal.NasalResonator_a2,
                          vtmLocal.NasalResonator_b0,
                          FNPinHZ,
                          BNPinHZ);
...
/*
    a1 = - exp( - 2 pi BW T )
	 ...
	 Radius = radius_table[(int)( 0.125 * 80/*BW*/ )]; 
     vtmLocal.Nasal_b1 = Radius * CosineTable[(int)( 0.125 * FZinHZ )]; 
     vtmLocal.Nasal_b2 = - Radius * Radius; 		 
	 vtmLocal.Nasal_b0= 1.0 -vtmLocal.Nasal_b2-vtmLocal.Nasal_b1;

/* Now convert to antiresonator coefficients (a'=1/a, b'=-b/a, c'=-c/a) */
/* It would be desirable to turn these divides into tables for chip impl. */
	Radius =  vtmLocal.Nasal_b0; //Use radius as a temp variable
	vtmLocal.Nasal_b0 = ( ( 1.0 )/ vtmLocal.Nasal_b0);
	vtmLocal.Nasal_b1 = -(vtmLocal.Nasal_b1/Radius);
	vtmLocal.Nasal_b2 = -(vtmLocal.Nasal_b2/Radius);
      ...
      vtmLocal.uiVoicePeriodSampleNumber++;
          ...
          vtmLocal.DifferentiatedVoicing = vtmLocal.DifferentiatedGlottalFlow ;
	      vtmLocal.uiVoicePeriodSampleNumber++;
    ...
    vtmLocal.DifferentiatedVoicing += vtmLocal.SpeakerBreathinessPitchSyncGain * Noiseb;
	...
	Output = vtmLocal.DifferentiatedVoicing;
    ...
    BIQUADRATIC_FILTER( Output,
		...
		MINIMUM_TWO_POLE_FILTER(vtmLocal.DifferentiatedVoicing,
                            vtmLocal.DifferentiatedVoicing/10,
							vtmLocal.NasalDelay_1,
							vtmLocal.NasalDelay_2,
							vtmLocal.NasalResonator_a1,
							vtmLocal.NasalResonator_a2,
			...
			//vtmLocal.iwave[uiNs] = (S16_T)( vtmLocal.DifferentiatedVoicing*10.0 );
    /******************************************************************/
    ...
    /******************************************************************/
//vtmLocal.iwave[uiNs] = (S16_T)( Output/10.0 );
	...
	// do a calculation for each).
		...
		if (Output<0)
		{
	    vtmLocal.iwave[uiNs] = (S16_T)-sabs;
		}
		else
		{
	    vtmLocal.iwave[uiNs] = (S16_T)sabs;
...
void read_speaker_definition(LPTTS_HANDLE_T phTTS)
{
  ...
  /********************************************************************/
   ...
   FLTPNT_T fnp=InverseSampleRateScale *FNP;  /*  Frequency of fixed nasal formant          */
 ...
 //DEBUGDEBUGDEBUG
  ...
  pVtm_t->PartialCascadePathGain = 32264.0 * c1_gain * c2_gain * c3_gain
