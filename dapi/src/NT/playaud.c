 ...
 * 028	MFG		02/27/2003	Merged in DECtalk version 5.00 changes
 *
 */
/**********************************************************************/
/*                                                                    */
...
{
#if !defined _WIN32 && !defined __linux__ // NAL warning removal
  LPDWORD pdwVol;
#endif
...
static void ClosePlayDevice( HPLAY_AUDIO_T pPlayAudio )
{
  ...
  else
  {
	...
	// MGS  The audio device seems to be lying when it says taht is is done playing.
...
#ifndef NOWIN
	  if (uiWriteLength & 0x01)			  
		  MessageBox(NULL, _T("Misalligned data"), _T("DECtalk"), MB_OK);
#endif
