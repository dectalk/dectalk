 ...
 * 008	mfg		22mar99	    include cemm.h for beginthread and endthread support for WinCE 211
 * 009	mfg		10oct00		include cemm.h for all CE builds
 * 010 	CAB		10/16/00	Changed copyright info
 * 010	MGS		02/08/2001	Fixed OP_WaitForEvent when it isn't supposed to wait forever
 * 011  MFG             02/28/2003 Merged in DECtalk version 5.00 changes
 *
 */
/**********************************************************************/
/*                                                                    */
                           ...
                           void * pThreadData )
{
  ...
  pThread = (HANDLE)_beginthreadex( NULL,
                                    StackSize,
                                    ThreadRoutine,
                                    pThreadData,
                                    0,
...
/* TQL 05/21/1997  change this for OSF build */
  ...
  if( pthread_getschedparam( *pThread, &iPolicy, &SchedParam ) != 0 )
  {
...
/* TQL 05/21/1997  change this for OSF build */
  ...
  if( 0 != pthread_getschedparam( *pThread, &iPolicy, &SchedParam ))
  {
    return TRUE;
  }
  else
