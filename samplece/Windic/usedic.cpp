...
//	char	fnam_d[80];
	...
	if (TextToSpeechReserved3(phTTS[tts_cur], 2, (char *)arpabet, arpa_size) < 0)
	{
	...
	*/
	...
	// CB ADDED
//#if (UNDER_CE == 200)
//	hTab = CreateFile(fnam_t, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
...
//#endif
	...
	*/
	...
	// CB ADDED
//#if (UNDER_CE == 200)
//	hDic = CreateFile(fnam_d, GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS,
//							  FILE_ATTRIBUTE_NORMAL, NULL );
	// setmode( int nMode = filebuf::text );
	// _fmode;

//	if (hDic == NULL)
//#else
	if ((fdic = _wfopen(fnam_d, L"w+b")) == NULL)
//#endif
	{
	...
	while( get_entry() )
	{
		if( !scan_entry())
		{
			...
			// CB ADDED to delete 0 byte file
			DeleteFile(fnam_d);
			return FALSE;
		}
