...
#endif
{
	...
	while (TRUE)
	{
		...
		if (pKsd_t->debug_switch == 0x2fff)
		{
			...
			printf("2010 -- not used              2020 -- not used\n");
...
/* GL 04/21/1997  change to be the same as the latest OSF code */
...
#ifdef ENGLISH_UK_Wrong_place
			/* GL 11/22/1997 support LINKRWORD */
			if ((buf[0] & ~PFONT) == LINKRWORD)
			{
		...
		if ((buf[0] & PFONT) == (PFCONTROL << PSFONT))
		{
			...
			//	printf("int ph %04x%04x\n",buf[1],buf[2]);
				pDph_t->wordclass[pDph_t->nsymbtot] = (buf[1] << 16) + buf[2];
				continue;
			}
...
#if defined (NWSNOAA) && defined (MSDOS)
				pDph_t->symbols[pDph_t->nsymbtot] = COMMA;
#else
...
#if (defined (WIN32) || defined (__osf__)) && !defined (i386)
