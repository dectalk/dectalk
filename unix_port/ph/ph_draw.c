 ...
 * 0033 jl		6/3/98			changed variable name bool to bool_short to comply with ANSI C++ compiler	
 */
...
/* swap _data is for develepment work only*/
//#define PH_SWAPDATA
...
#if defined PH_REGRESSION_TEST || defined PH_SWAPDATA
...
#include "dbgwins.h"
#endif
void r_modulation (LPTTS_HANDLE_T phTTS,short *formpointer, short percent, short bool_short); /* jl 6/13/98 */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
/* D R A W P A R S                                  					 */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
 
...
void phdraw (LPTTS_HANDLE_T phTTS)
{
	...
	short                   temptilt;
#ifdef GERMAN
	short                     vadjust[] = {0, 0, 0, 0, 2, 3, 4, 5, 6, 7, 8, 8, 8, 8, 8, 8};
#endif
#ifndef MSDOS
	/* MGS 10/14/1997 BATS #470 Fixed crashed due to reset */
...
/* Spdef par FT -> 0 < f0_dep_tilt < 100, temptilt = 1/40 (max) of 1400-f0 */
	...
	*parp += (pDph_t->spdeftltoff - 6);		/* Add in spdef offset */
	...
	{								   /* Set in ph_setar.c */
		...
		{							   /* Do only if AV on  */
			...
			value = frac4mul (pDph_t->spdeflaxprcnt, (pDphsettar->breathyah + 30));
...
#endif
#ifdef SPANISH
				pDph_t->parstochip[OUT_AP] = 0;	/* value; eab help */
#endif
			}
#ifdef SPANISH
			value=0;
	...
	if (pDph_t->parstochip[OUT_AV] > 3)
	{
		...
		/* EAB OLD CODE 
		 * temptilt = (*parp>>1) ;  
		 * printf(" *parp %d tilt %d  vadj %d   \n",*parp,parstochip[OUT_TLT],vadjust[temptilt]);
		 * parstochip[OUT_AV] += vadjust[temptilt];
		 */
...
#ifdef GERMAN
	/* Compensate for loudness decrease when strongly tilted 4/96 EAB Modified to do its job better */
	if (pDph_t->parstochip[OUT_AP] > 3)
		pDph_t->parstochip[OUT_AP] -= 3;	   /* reduce to balance reduction in av */
/* eab 4/16/98 If tilt av code vadjust code is not in then this isn't needed oliver*/
	if (pDph_t->parstochip[OUT_AV] > 3)
	{
		pDph_t->parstochip[OUT_AV] -= 3;	   /* reduce to prevent overloads on next calc. */
		/* gf reduces in phvset for same reson */

		temptilt = (*parp >> 1);

#ifdef DEBUGPHT
#if defined (WIN32) && defined (PRINTFDEBUG)
WINprintf(" *parp %d tilt %d  vadj %d   \n",*parp,pDph_t->parstochip[OUT_AV],vadjust[temptilt>>1]); 
#endif
#endif
//		pDph_t->parstochip[OUT_AV] += vadjust[temptilt>>1];
	}
	/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
...
/* GL 1/16/1997, this following code will never work for Spanish */
#ifdef SPANISH
	/* Compensate for loudness decrease when strongly tilted 4/96 EAB Modified to do its job better */
	if (pDph_t->parstochip[OUT_AP] > 3)
		pDph_t->parstochip[OUT_AP] -= 3;	   /* reduce to balance reduction in av */
...
//		pDph_t->parstochip[OUT_AV] += vadjust[temptilt>>1];
	...
	if (pDph_t->allophons[pDphsettar->np_drawt0] == E_R)
	{
		r_modulation (phTTS,pDph_t->param[A2].outp, N20PRCNT, FALSE);
		r_modulation (phTTS,pDph_t->param[A3].outp, N20PRCNT, FALSE);
		r_modulation (phTTS,pDph_t->param[AV].outp, N8PRCNT, FALSE);
	}
	/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
	/* if current phone is uvular /RR/, modulate several parameters      */
	/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
	...
	else if (pDph_t->allophons[pDphsettar->np_drawt0] == E_RR)
	{
		...
		r_modulation (phTTS,pDph_t->param[A2].outp, N20PRCNT, FALSE);
		r_modulation (phTTS,pDph_t->param[A3].outp, N20PRCNT, FALSE);
		r_modulation (phTTS,pDph_t->param[AP].outp, N70PRCNT, TRUE);
	}
	else
		/* Initialiatize the modulation counter eab 2/6/98*/
		pDph_t->modulcount =0;
...
#ifdef plosivevocalstart		
		if(pDph_t->parstochip[OUT_AP] >0 && pDph_t->parstochip[OUT_AV] ==0  )
		{
			pDph_t->sinstart=800;
		}
		if( pDph_t->parstochip[OUT_AV] > 0)
			if(pDph_t->sinstart <4000 )
				pDph_t->sinstart+=200;
			else
				pDph_t->sinstart=0;
		pDph_t->parstochip[OUT_GS] += pDph_t->sinstart;
#endif
	...
	if((featb[ pDph_t->allophons[pDphsettar->np_drawt0]] & FNASAL) IS_PLUS)
	{
		if(pDph_t->malfem == MALE)
		{
			...
			//pDph_t->parstochip[OUT_FNP] = 969;
			//pDph_t->parstochip[OUT_FZ] += 180;
		}
		else
		{
				/* eab 4/23/98 If phoneme is less than M then it's a nasalized french vowel*/
	...
	if ((pDph_t->allophons[pDphsettar->np_drawt0+1] == SIL)
		...
		/* eab 4/5/98 NFor 5.0 all languages should do this*/
		/* In a lateral like l av jumps whem mouth and tongue open tract*/
...
#ifdef GERMAN
			if( (pDph_t->allophons[pDphsettar->np_drawt0] == RR) &&
				((featb[ pDph_t->allophons[pDphsettar->np_drawt0]] & FVOWEL) IS_PLUS))

				pDph_t->parstochip[OUT_AV] +=4;

#endif

#ifdef PH_SWAPDATA
        if (PHSwapIn == NULL && PHSwapOut == NULL)
        {
                PHSwapIn = fopen("phdata.in","rb");
                PHSwapOut = fopen("phdata.out","wb");
                fprintf(PHSwapOut," AP   F1   A2   A3   A4   A5   A6   AB  TILT  T0   AV   F2   F3    FZ   B1   B2   B3    \r\n");
                if (PHSwapIn != NULL)   fscanf(PHSwapIn," AP   F1   A2   A3   A4   A5   A6   AB  TILT  T0   AV   F2   F3   FZ   B1   B2   B3\r\n");
        }
        if (PHSwapOut != NULL) {
                for (ii = 0; ii < 16; ii++) {
                        fprintf(PHSwapOut,"%4d ",pDph_t->parstochip[ii]);
                }
                fprintf(PHSwapOut,"\r\n");
        }
        if (PHSwapIn != NULL) {
                for (ii = 0; ii < 16; ii++) {
                        fscanf(PHSwapIn,"%4d ",&pDph_t->parstochip[ii]);
                }
                fscanf(PHSwapIn,"\r\n");
                if (feof(PHSwapIn)) {
                        fclose(PHSwapIn);
                        PHSwapIn = fopen("phdata.in","rb");
                        fscanf(PHSwapIn," AP   F1   A2   A3   A4   A5   A6   AB  TILT  T0   AV   F2   F3   F4   FZ   B1   B2   B3   B4   PH   DU   DP  OPENQ FNP  FNF  GNF GSPD\r\n");
                }
        }
#endif
	...
	if( pDphsettar->phcur != 0)
	{
		...
		for (ii = 0; ii<=20; ii++)			   /* EAB FOR REGRESSION TESTING */
		{
			WAIT_PRINT;
#if defined (WIN32) && defined (PRINTFDEBUG)
