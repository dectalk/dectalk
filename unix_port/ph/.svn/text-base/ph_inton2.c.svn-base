...
short f0_mphrase_position[] = {   110, 90, 60, 40, 20 };
/* Was: 210,  90, 40, 20 */
...
short f0_fphrase_position[] = {   160, 120, 96, 70, 30 };
/* Was: 210,  90, 40, 20 */
...
short f0_mphrase_position[] = {
 80,  110,  80,  60,  40,  20,  0,  0 };

short f0_mstress_level[] = {
 1,  101,  71,  190,  0,  0,  0,  0 };
...
fbfound:
			...
			|| (pDph_t->f0mode == SINGING))
		{
			...
			{
				make_f0_command (pDph_t, USER,0, (2000 + pDph_t->user_f0[nphon]), 0, 0, &cumdur,nphon);
			}
			goto skiprules;
		}
...
#if !(defined  GERMAN || defined ENGLISH_US) /* EAB 4/13/98 German doesn't use the Hat_rise hat fall pattern*/
		...
		if ((pDph_t->f0mode == NORMAL) || (pDph_t->f0mode == HAT_F0_SIZES_SPECIFIED))
		{
			...
			not on last stresssed syllable as american english does*/
					...
					if( pDph_t->number_words > 2  )
					{
						...
						if((pDph_t->nallotot -nphon) <6)
						{
							if (nphon == nphontmp)
							{
								make_f0_command (pDph_t,GLIDE, 23,-200, -(pDph_t->allodurs[nphon-1]), pDph_t->allodurs[nphon-1], &cumdur, nphon);
								make_f0_command (pDph_t,GLIDE, 23, +250, 0, pDph_t->allodurs[nphon], &cumdur, nphon);
								pDph_t->done =1;
							}
						}
					}
			...
			/* eab 1/19/98 previous target was upglide so now we
										want to do a downglide */
				{
					...
					make_f0_command (pDph_t,GLIDE, 20,targf0 , -6,pDph_t->allodurs[nphon], &cumdur, nphon);
					make_f0_command (pDph_t,GLIDE, 21, pDph_t->prevtargf0>>3, pDph_t->allodurs[nphon],pDph_t->allodurs[nphon+1], &cumdur, nphon);
					goto skiprules;
				}
	...
	
				{
					...
					make_f0_command (pDph_t,STEP, 1, pDphsettar->hatsize<<1, delayf0,20, &cumdur, nphon);
...
#if defined NWSNOAA  || defined GERMAN  /*maybe in all cases*/
...
/* eab 2/21/97 EAB remove emphasisflag and associated stuff pulled see note in phsort.c */
					...
					if( pDph_t->number_words > 2 )
					{
						...
						{
							make_f0_command (pDph_t,GLIDE, 22, targf0, -6,pDph_t->allodurs[nphon], &cumdur, nphon);
					...
					/* Save stress impulse in command string */
						make_f0_command (pDph_t,IMPULSE, 2, targf0, delayf0,20, &cumdur, nphon);
...
/*			  i.e. readjust timing so get fall-rise, not r-f-r */
				...
				lowrisesw = 0;
					...
					make_f0_command (pDph_t,STEP, 3, -f0fall , delayf0, 20, &cumdur, nphon);
...
#else
						make_f0_command (pDph_t,IMPULSE, 41,F0_QGesture1, delayf0,24, &cumdur, nphon);
						make_f0_command (pDph_t, IMPULSE, 41,F0_QGesture2, pDph_t->allodurs[nphon],24, &cumdur, nphon);
#endif 
					}
					else
					{
						...
						{
						make_f0_command (pDph_t, IMPULSE,42,  F0_CGesture1, 3 ,22, &cumdur, nphon);
						make_f0_command (pDph_t, IMPULSE, 42, F0_CGesture2, (pDph_t->allodurs[nphon]>>1),18, &cumdur, nphon);
						}
#else
						if(pDph_t->commacnt == 0)
						{
							make_f0_command (pDph_t, IMPULSE,420,  F0_CGesture1, delayf0,24, &cumdur, nphon);
							make_f0_command (pDph_t, IMPULSE, 420, F0_CGesture2, (pDph_t->allodurs[nphon]>>1),24, &cumdur, nphon);
						}
#endif
						else
						{
							make_f0_command (pDph_t, IMPULSE,421,  F0_CGesture1, delayf0,24, &cumdur, nphon);
						}
						pDph_t->commacnt++;
...
work since I believe 1984*/
				...
				if (pDph_t->had_hatend && (feacur & FSYLL) IS_PLUS)
				{
					...
					make_f0_command (pDph_t,STEP, 31, -f0fall, delayf0,0, &cumdur, nphon);
					pDphsettar->hat_loc_re_baseline -= f0fall;
				...
				{					   /* or 1-str nofall */
							...
							|| (featb[phonex] & FCONSON) IS_PLUS))
					{
							...
							|| ((struccur & FBOUNDARY) == FEXCLNEXT))
						{
							...
							{
								make_f0_command (pDph_t,GLOTAL, 5, targf0>>1, pDph_t->allodurs[nphon] - NF30MS, 30, &cumdur, nphon);
							}
							else
#endif

							make_f0_command (pDph_t,GLOTAL, 5, targf0-50, 0 , 40, &cumdur, nphon);
						}
						...
						if ((struccur & FBOUNDARY) == FQUENEXT)
						{
							...
							make_f0_command (pDph_t,IMPULSE, 6, F0_QGesture1, delayf0, 24, &cumdur, nphon);
							make_f0_command (pDph_t,GLIDE, 6, F0_QGesture2,0, pDph_t->allodurs[nphon], &cumdur, nphon);
#else
								/* Unstressed vowel followed by a question mark */
							make_f0_command (pDph_t,IMPULSE, 6, F0_QGesture1, delayf0, 24, &cumdur, nphon);
							make_f0_command (pDph_t,IMPULSE, 6, F0_QGesture2, pDph_t->allodurs[nphon],20, &cumdur, nphon);
#endif
						...
						if ((struccur & FBOUNDARY) == FCBNEXT)
						{
							...
							delayf0 += NF20MS;
							make_f0_command (pDph_t,IMPULSE, 6, F0_CGesture1, delayf0, 24, &cumdur, nphon);
							make_f0_command (pDph_t,IMPULSE, 6, F0_CGesture2, pDph_t->allodurs[nphon], 20, &cumdur, nphon);
						}
...
#endif							
							make_f0_command (pDph_t,GLOTAL, 5, targf0, pDph_t->allodurs[nphon] - NF100MS, 20, &cumdur, nphon);
						}
...
GERMAN
					if (pDph_t->cbsymbol)
						make_f0_command (pDph_t, 7, +180, 20, &cumdur, nphon);
					else
#endif  */
#ifndef GERMAN

					make_f0_command (pDph_t,STEP, 7, -(pDphsettar->hat_loc_re_baseline), 0, 20, &cumdur, nphon);
				...
				{
					make_f0_command (pDph_t,F0_RESET, 7, 0, 0, 0, &cumdur, nphon); /* RESET! */
					issubclause = TRUE;
				}
				...
				{
					pDph_t->commacnt=0;
					make_f0_command (pDph_t,F0_RESET, 8, 0, 0, 0, &cumdur, nphon);
					pDphsettar->hat_loc_re_baseline = 0;
	...
	if(nphon > 0 && nphon < pDph_t->nallotot-1)
...
#ifdef NWSNOAA /*maybe in all cases*/
