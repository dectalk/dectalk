 ...
 *    Revision History:
 *
 * 12 Jul 94	...tek		changed the initial flush to start_flush()
 * 13 dec 95	...tek		edits to merge with common code pool 
 * 26 apr 96	...tek		fix EBD's inability to produce a tone before
 *							speaking..
 * 11 nov 96    ...gl       add language support by using the dic_lang flag.
 * 27 nov 96    ...gl       use [:power lspeak] to send power status to in_ring
 * 25 sep 97    ...gl       use array for all dictionary entry
...
#ifdef ENGLISH_US
        #ifdef US_AND_SP
unsigned char welcome[]="DECtalk Express is running for English and Spanish.[:sync][:power lspeak]\n";
        #else
unsigned char welcome[]="DECtalk Express is running.[:sync][:power lspeak]\n";
        #endif
#endif
