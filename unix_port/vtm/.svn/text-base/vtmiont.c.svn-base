...
#ifdef _DEBUG // tek find out when the input pipe is going dry..
		{
			char szTemp[256];
...
{	PVISUAL_DATA pvdPacket = NULL;
	...
	if (pvdPacket) // just punt if the malloc failed
	{
		...
		{
			char szTemp[256]="";
			sprintf(szTemp,"SendVisualNotification (BM): pho:%02lx (%08lx) [%c], dur:%04ld, samp:%lxx TFC:%ld. at %lu\n",
				(DWORD)pvdPacket->cEnginePhoneme,
				dwPhoneme, 
				pvdPacket->cEnginePhoneme,
				dwDuration,
