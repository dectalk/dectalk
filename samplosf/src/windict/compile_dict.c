 ...
 * HISTORY
	6	08/01/97	leeber	SNAPSHOT106  V4.41 R010 11/17/97 NWSNOAA Beta2 Trail Code, tested (16 & 32 bit) 11/17/97 cjl
	5	08/01/97	leeber	Build 106 V4.41.08 R009 10/31/97 NWSNOAA Beta1 Code Release, tested (16 & 32 bit) 10/31/97 cjl
	4	08/01/97	leeber	SNAPSHOT (Pre-R009 that will include buildable NWS/NOAA and code ONLY English UK). Also represents latest working (verified) ML for Intel/Windows and Alpha UNIX ONLY. Alpha NT has a build bug. 10/27/97 cjl
	3	08/01/97	leeber	CODE MARK POINT (Pre-R009 that will include buildable NWS/NOAA and code ONLY English UK). Also represents latest working (verified) ML for Intel/Windows and Alpha UNIX ONLY. Alpha NT has a build bug. 10/27/97 cjl
...
**
**     Sep 27, 94   KM  Added parseString and the routines it calls
**                      to validate input strings.
**     Oct 06, 94   KM  Changed entry names to avoid conflict with DECtalk
**                      routines.
**
****************************************************************************/
...
unsigned char *arpabet=usa_arpa;
#endif
#ifdef ENGLISH_UK
#include "uk_def.h"
#include "uk_phon.tab"
unsigned char *arpabet=uk_arpa;
#endif
#ifdef GERMAN
#include "ger_def.h"
#include "ger_phon.tab"
unsigned char *arpabet=german_arpa;
#endif
#ifdef SPANISH
#include "spa_def.h"
#include "spa_phon.tab"
unsigned char *arpabet=spanish_arpa;
#endif
...
int get_entry(char *text)
{
   ...
   if (text[idx] != '\0')
   {
      while( text[idx] != ']' )
      {
         /* X Windows sticks 0xa at the end of each line */
         if ( text[idx] != 0xa ) 
            line[j++] = text[idx++];
         else 
            idx++;
...
int parseString( char *text )
{
   ...
   strcpy( tempStr, text);
   strcpy(postfixStr, strstr( tempStr, "["));
