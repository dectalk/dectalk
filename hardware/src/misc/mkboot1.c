...
char *argv[];
{
   ...
   char *rsymb;
	...
	FILE *fexe,*fmap,*fabs,*fstb,*ftmp,*fh;
   fpos_t   fpos;
	char fnam[80];
	char filenam[80];
	char segname[80];
 ...
 */
	...
	strcpy(&fnam[pos],".hex");
	if ((fh=fopen(fnam,"w")) == NULL)
		{
		printf("  Can't open hex file name : %s",fnam);
		exit(1);
		}
 ...
 */
   ...
   printf("\n  Writing hex high and low byte files...");
	write_hex(ftmp,bytes_out,&input_string[0],fh,start_cs,start_ip);
	fclose(fh);
...
write_hex(fabs,size,buff,fh,cseg,cip)
FILE *fabs,*fh;
long	size;
unsigned char buff[];
...
/*
 *  write out the  bytes from this chunk ...
 */
