	...
	while (p!=NULL)
	{
		...
		if (p->data->special_rule!=NORMAL_TAG_VALUE)
		{
			...
			if (p->data->special_rule==GORET_TAG_VALUE)
				temp|=BIN_GORET;
			*((U16 *)&(p->data->binary_data[rule_length]))=temp;
			rule_length+=2;
			*((U16 *)&(p->data->binary_data[rule_length]))=p->data->rule_number;
			rule_length+=2;
		}
		else
		{
			...
			*((U16 *)&(p->data->binary_data[rule_length]))=p->data->rule_number;
			rule_length+=2;
			*((U32 *)&(p->data->binary_data[rule_length]))=p->data->lang_flag;
			rule_length+=4;
			*((U32 *)&(p->data->binary_data[rule_length]))=p->data->mode_flag;
			rule_length+=4;
			if (p->data->next_hit_rule!=-1)	
			{	
				*((U16 *)&(p->data->binary_data[rule_length]))=p->data->next_hit_rule;
				rule_length+=2;
			}
			if (p->data->next_miss_rule!=-1)	
			{	
				*((U16 *)&(p->data->binary_data[rule_length]))=p->data->next_miss_rule;
				rule_length+=2;
			}
			if (p->data->next_goret_hit!=-1)
			{	
				*((U16 *)&(p->data->binary_data[rule_length]))=p->data->next_goret_hit;
				rule_length+=2;
			}
			if (p->data->next_goret_miss!=-1)
			{	
				*((U16 *)&(p->data->binary_data[rule_length]))=p->data->next_goret_miss;
				rule_length+=2;
			}
			i=0;
			k=-1;
...
void write_output(rule_node list,dict_names dict_list,int *rule_sections,char*infile_name,char *outfile_name)
{
	...
	fprintf(outfile_asc,"\n");
	fprintf(outfile_asc,"int num_rule_sections=%d;\n\n",rule_sections[0]);
	fprintf(outfile_asc,"int rule_sections[%d]={",rule_sections[0]);
	for (i=1;i<=rule_sections[0];i++)
	  if (i==rule_sections[0])
		...
		fprintf(outfile_asc,"\t%d,",rule_sections[i]);
	fprintf(outfile_asc,"\t};\n\n");
	fprintf(outfile_asc,"int num_rules=%d;\n\n",rule_count+1);

	fprintf(outfile_asc,"int rule_index_table[%d] = {\n",rule_count+1);
...
//			s[i].num_entries=p->num_entries;
	...
	if (num_dictionaries==0)
	  {
		fprintf(outfile_asc,"dict_pointers_t dict_point[1]={{0,0}};\n\n");
	  }
	else
	  {
	...
	if (num_dictionaries==0)
	{
	  fprintf(outfile_asc,"int dict_index_table[%d] = {\n",1);
