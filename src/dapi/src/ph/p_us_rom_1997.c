 ...
 * 0002 DGC		12/27/1984	    Added some "readonly" things for the 80186, as
 *                      		these tables must go into the ROM.
 * 0003 DK		02/11/1985	    Backed off amplitudes of nasals a bit, voice [bdg]
 *                       		replaced [EM] by [D$] (dentalized d)
 * 0004 DK		03/01/1985      Improve place[] array, free up two featb[] feature bits
 * 0005 DK		04/12/1985     	Improve bursts for telephone intelligibility
 * 0006 DK		05/13/1985      Add getcosine[] table
 * 0007 DK		07/17/1985      AV target for nasal -2 db, voiced fric -3, dx - 6
 * 0008 DK		07/25/1985      Add lineartilt[] array
 * 0009 DK		07/29/1985      Add feature FSTOP
 * 0010 DK		08/26/1985      Tweeked vowels incrementally
 * 0011 EAB		11/20/1989      REMOVE TROJAN HORSE (DBMESS)
 * 0012 EAB		03/24/1994      	modified iy w and nasal so different that it needs 
 *								more analysis this is a test.
 * 0013 EAB		03/18/1995      Intervocalic d i.e. dx was not good with front vowels
 * 								Found hardcoded pointer error
 *								as well as the need for acoustic tuning.
 * 0014 EAB		03/19/1996      Found that female was off too, got an insite to treat
 ...
 * 0015 EAB    	04/5/1996   		Found a conceptual error that  B2 B3 for silence was
 *                              set to values it's much better to have it set in context
 *                              it was causing overloads dir to a too rapid change in bandwidth
 * 0016	MGS		06/06/1996		Changed file name frpm phrom.c to p_us_rom.c
 * 0017 SIK		07/12/1996  	Cleaning up and maintenance
 * 0018 EDB		01/10/1997		Change B3 target.
 * 0019	EAB		06/30/97		Fixed female targets to cure mis-perception of minor, improved nasal formants,
 *								ay vowel targets, and femloc for rr plus rr froamnt targets
 *								Fixes BAT 396
 * 0020 EAB		10/15/97		Slightly reduce inherent duration of EN to help correect a problem (safe)
 *								and also changed inherent duration of Q ,a previously unused phone
 * 0021 EAB             11/6/97                 Tuned F sound in response to NOaas complaint of dificulty between fifty and sixty.
 * 0022 CJL             11/07/97        Merged by adding NWSNOAA symbol.
 * 0023 EAB		11/18/97		B locus bad cusing front vowels with low f2's to sound
 *								like d i.e cab vs cad BATS 527 probably bad for female as well
 *								 but this hasn't been checked for yet
 *
 */
 ...
 * *      next-to-last number is value at end of diphthong *      last value is minus 1 to signal end of diphthong */
...
/* MALLINE is defined in PH and is the number of parameters * *in a line of malleloc */
...
#endif
{
	0,
	  /* [F] front,back */
#ifndef NWSNOAA //***Non-NWSNOAA***cjl 11/07/97
        300, 63, 30, 1, 92, 35, 2080, 35, 30,
#else           //***For NWSNOAA***cjl 11/07/97
	300, 63, 30, 1600, 72, 35, 1980, 35, 30,
#endif
...
/* Names for all (phonemic) input symbols: SI    IY    IH    EY    EH    AE    AA    AY    AW    AH AO    OW    OY    UH    UW    RR    YU    AX
 * IX    IR ER    AR    OR    UR     W     Y     R     L    HX    RX LX     M     N    NG    EL    D$    EN     F     V    TH DH     S     Z    SH
...
#ifdef PHEDIT2
short                   f0_phrase_position[20] =
#else
short                   us_f0_phrase_position[] =
#endif
{210, 90, 60, 40, 0};
...
#ifdef PHEDIT2
short                   f0_stress_level[20] =
#else
short                   us_f0_stress_level[] =
#endif
{1, 71, 31, 281};
...
#endif
{
	...
	-348, -366, -384, -402, 350, 320, 350, 360, -2, 490,
	  ...
	  /* LX    M    N   NG   EL   D$   EN    F    V   TH */
		480, 450-100, 466, 450, 480, 220, 450, 360, 300, 340,
	...
	-354, -372, -390, -408, 800, 2490, 1033, 1000, -1, 1600,
	  ...
	  /* LX    M             N   NG   EL   D$   EN    F    V   TH */
	930, 1340, 1500 + 470, 2056, 990, 1860, 1530, 1150, 1150, 1530,
	...
	-360, -378, -396, -414, 2630, 3400, 1700, 3430, -1, 1950,
	  ...
	  /* LX    M    N   NG   EL   D$   EN    F    V   TH */
		3320, 2590, 2900 + 650, 3100, 3320, 3020, 2920, 2460, 2460, 2940,
...
#endif
{
	...
	2990, 40, 2990, 200, 2990, -1,
	  /* [AY] 104 f1,f2,f3,b1 */
	846, 40, 800, 120, 730, 220, 590, -1,
	1380, 90, 2070, 200, 2400, -1,
	2660, 150, 2680, 200, 2740, 240, 2817, -1,
	...
	2800, 40, 2700, 180, 2700, -1,
	  ...
	  /* [RR] 276 f1,f2,f3 */
	691, 30, 691, 170, 691, -1,
	1580, 30, 1600, 170, 1600, -1,
	2160, 30, 2080, 170, 2080, -1,
	  /* [AX] 294 f1,f2,f3 */
	720, 30, 750, 90, 750, -1,
...
#endif
{
...
#else           //***For NWSNOAA***cjl 11/07/97
	350, 65, 30, 2000, 77, 40, 2300, 35, 30,
	350, 75, 30, 1800, 75, 30, 2000, 65, 40,
#endif
	  /* [V] front,back */
	...
	450, 20, 30, 1, 88, 40, 1750, 80, 30,
	  /* [N] front,back-unrounded,rounded  450,24,35,1500,75,35,3000,35,45, 450,22,30,1900,25,75,3040, 0,60, 450,24,30,1900,25,80,2890, 0,80,       */

		/* [N] front,back-unrounded,rounded */
	450, 24, 35, 2300, 75, 35, 3200, 35, 45,
	450, 22, 30, 2200, 65, 75, 3140, 30, 60,
	450, 24, 30, 2160, 65, 80, 2590, 30, 80,
	...
	480, 20, 40, 1650, 42, 70, 2500, 25, 70,
	  /* [DX] [FX] front,back-unrounded,rounded  370,33,35,1650,60,35,2350,50,45, 370,33,45,1650, 30,80,2390,50,50, 370,33,40,1600,
	   * 30,80,2300,50,100,         */
	  /* [DX] [FX] front,back-unrounded,rounded 4/13/95 eab chnaged drastically it was way off */
...
/* 250,43,35,1340+400,66+30,35,2090+300,60,45, 250,43,50,1340,40,35,2090,61,50, 250,43,40,1340,40,35+40,2090,62-20,100, */
...
#endif
{
	...
	0, 0, 0, 0, 54, 60,
	0, 0, 0, 0, 54, 60,
	  /* OUT            39,  47,  54,   0,   0,   0, 36,  44,  51,   0,   0,   0,  END OUT */
	  /* [D] */
	0, 0, 0, 0, 49, 55,				   /* was 0, 63 */
	0, 0, 0, 0, 49, 55,
	0, 0, 0, 0, 49, 55,
	0, 0, 0, 0, 49, 55,
	  /* OUT            35,  43,  50,   0,   0,   0, 32,  40,  47,   0,   0,   0,  END OUT */
	  /* [K] */
	0, 44, 51, 0, 35, 0,
	45, 0, 0, 0, 0, 0,
	48, 0, 30, 0, 0, 0,
