...
/* if ((pDphsettar->phonex == N  || pDphsettar->phonex == M) && ((phone_temp & PVALUE) != R )) { tartemp = 300; pDphsettar->nasvowel=1; }        */
	}
	/* GETTAR:  AV, AP */
			   				...
			   				short struccur, short pholas, short struclas)
{
	...
	if ((feacur & FBURST) IS_PLUS)
	{
		...
		{
			if (us_place[pDphsettar->phcur & PVALUE] == us_place[pDphsettar->phonex & PVALUE])
			{						   /* Was >= */
				bdur = 0;
			}
		}
