 ...
 * 0029 EAB		4/6/98			#endif in wrong place at dummy_v_dur
 */
/* #define MSDBG4 */
...
extern short us_f0_phrase_position[];		/* F0 rise as f(phrase-position) */
//extern short us_featb[];	   /* Phonetic features            */
//extern us_begtyp[];
...
//#if defined (ENGLISH) || defined (GERMAN)
				...
				targf0 += us_f0_phrase_position[pDphsettar->nrises_sofar];
...
/* eab 2/21/97 EAB remove emphasisflag and associated stuff pulled see note in phsort.c */
					...
					if( pDph_t->number_words > 2 )
					{
						...
						{
							make_f0_command (pDph_t,GLIDE, 22, targf0, -6,pDph_t->allodurs[nphon], &cumdur);
...
//#if defined ENGLISH_US || defined GERMAN
	  ...
	  skiprules:					   /* END OF F0 RULES */
			...
			&& (pDph_t->nallotot < NPHON_MAX))
		{
			...
			else if (pholas == SP_YX)
				pDph_t->allophons[nphon + 1] = SP_I;
			else if (pholas == SP_YX)
				pDph_t->allophons[nphon + 1] = SP_I;
			else
			{
				pDph_t->allophons[nphon + 1] = SP_A;
			}
...
/*************************end of ph_inton.c**************************************/
#endif /*ifdef NWSNOAA || ENGLISH_UK */
