 ...
 *  18. 11/03/1998 MGS  BATS #782
 *  19	11/12/1998	ETT	you gotta initialize!  esp. floating points!
 *  20	11/20/1998	GL	BATS#828 use VTM_DEBUG to replace _DEBUG
 *  21  12/16/98	BATS 845 k1 k2 mishandled causing bad nopen values
 */
...
#include "vfmfiltr0.h"    /*  Filters Macros                            */
#include "vfmd2pol0.h"    /*  Two pole filter design Macro              */
#include "vtminst.h"
...
#define SWG_INIT_VALUE 0 //(12345)
  ...
  for ( uiNs = 0; uiNs < vtmLocal.uiNumberOfSamplesPerFrame; uiNs++ )
  {
    ...
    for ( nsr4 = 0; nsr4 < 4; nsr4++ )
    {
...
#ifdef NEW_VTM
		if(vtmLocal.LaryngealSourcePitchSyncGain > 40 && glotnoise == 0.0)
		{
			glotnoise= vtmLocal.SpeakerBreathinessPitchSyncGain *32000.0;
		}
#endif
      ...
      if ( vtmLocal.uiVoicePeriodSampleNumber == (int)vtmLocal.T0 )
      {
        ...
        if ( vtmLocal.LaryngealSourceGain > 0.0 )
          vtmLocal.uiPositionToStartNoiseModulation = (int)( 0.5 * vtmLocal.T0 );
...
#ifdef NEW_VTM
	{
	 ...
	 if(glotnoise > 300.0)
		 glotnoise -= 300.0;
	else if(glotnoise > 0.0)/* eab 4/16/98 for debug kater can be just else*/
		glotnoise = 0.0;
...
void read_speaker_definition(LPTTS_HANDLE_T phTTS)
{
 ...
 //DEBUGDEBUGDEBUG
  ...
  pVtm_t->k2 = (FLTPNT_T)pSpeakerDefinition->nopen2; /* 15 */
