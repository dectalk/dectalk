 ...
 *		002		CAB	06/22/201	Updated copyright info
 */
...
{	
	...
	fprintf(outfile," ***********************************************************************\n");
	fprintf(outfile," *\n");
	fprintf(outfile," *                           Copyright (c)\n");
	fprintf(outfile," *    Copyright © 2000, 2001 Force Computers, Inc. ASolectron Company. All rights reserved.\n");
	fprintf(outfile," *\n");
	...
	fprintf(outfile," *    technology of Force Computers Incorporated and other parties.\n");
	fprintf(outfile," *    Possession, use, or copying of this software and media is authorized\n");
	fprintf(outfile," *    only pursuant to a valid written license from Force or an\n");
	...
	else if (head->word_ending & 0x04)
	{
		if (head->written==0)
		{
			...
			for (i=0;i<p->length;i++)
			{
				...
				{
					temp_schars[i].entry=p->schars[i].schar->index_number;
				}
			}
	...
	else
	{
		if (head->written==0)
		{
			...
			for (i=0;i<count_chars;i++)
			{
				...
				temp_chars[i]=head->chars[i]->index_number;
			}
