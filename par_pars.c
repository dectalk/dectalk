 ...
 *
 *                           Copyright (c)
 *    (c) Digital Equipment Corporation 1996, 1997. All rights reserved.
 *
...
extern int num_rule_sections;
extern int rule_sections[];
extern int num_rules;
extern int rule_index_table[];
extern unsigned char rule_data_table[];
extern dict_pointers_t dict_point[];
extern int dict_index_table[];
...
unsigned int char_type_table[16] = {
	TYPE_null,
	TYPE_alpha_num,
	TYPE_alpha,
	TYPE_any_char,
...
int main(void)
{
	...
	{                                       			
		...
		*/
		...
		par_process_input(pKsd_t,input_array,new_input_array,output_array,dict_hit_array,input_indexes,new_input_indexes,output_indexes,0x00000001,0x00000FFF,2,0,&match_array,&ret_value);
#ifndef NO_PRINT
		par_print_output(output_array,output_indexes);						/* print the results of the matching */
#endif
#endif
 ...
 *	current search left off in the search of the array
	*/
	...
	unsigned int		current_rule_R_value;
#endif
...
//			memset(match_array->array_lengths,0,sizeof(match_array->array_lengths));
			current_rule=&(rule_data_table[rule_index_table[current_rule_number]]); /* this line will change for the new rule tables */
			rule_p=0;
#ifdef DEBUG
 ...
 */
#ifdef DEBUG
int par_get_return_level(int *return_rule,
						   int *return_level,
						   int current_rule_number)
{
	if (*return_level>0)
 ...
 */
#ifdef DEBUG
void par_set_return_level(int *return_rule,
						  int *return_level,
						  int go_rule)
{
					...
					int dict_state_flag)
{
	...
	U8 *new_rule;
	int in_rule_index;
	int next_rule_number,rule_p,length=0;
	int length_of_input;
	int temp;
	U8 end_of_match;
//	U8 end_of_action;
						  ...
						  
{
	...
	int dict_number;
	int value;
	int rev_same=0,for_same=0,npos;
	int save_for;
#ifdef DEBUG
						   ...
						   int in_rule_index)
{
	...
	*/
	...
	while (i<ret_value->output_offset && ((has_cons & has_vowel)==0))
	{
 ...
 *		INPUT	unsigned char 	*current_rule	pointer to the current rule
 *		INPUT	unsigned char 	*input_array	the input array
 *		INPUT	int				save_p			The number of sections marker
