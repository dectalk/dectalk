...
LTrie make_new_ltrie(const U16 nnodes, const U16 increments, const U16 type)
{
  LTrie newtrie;      
...
void file_dump_ltrie(const LTrie Lt, const GERLETTER *name, const int number, FILE *outstr, const char *nameextension)
{
  ...
  for (i=0; i<Lt.last; i++)
    {
	fprintf(outstr, "      { 0x%02X, %2i, %4i, %6i, %6i, %8i}, \n",
		  Lt.Nodes[i].letter,
		  Lt.Nodes[i].info,
		  Lt.Nodes[i].rule,
		  Lt.Nodes[i].down,
...
BOOL contains_umlaut(const GERLETTER *pin)
{
  int i;
  for (i=0; pin[i] != '\0'; i++)
    {
	  ...
	  || pin[i] == UML_o
	  || pin[i] == UML_u
	     || pin[i] == UML_sz) return 1;
    }
  return 0;
}

GERLETTER *convert_to_nonumlaut(GERLETTER *pin, GERLETTER *conv)
{
  GERLETTER *pout, *p;
  GERLETTER c;
  p = pin;
...
LTrie file2ltrie(const GERLETTER *filename, PhonemeWords *Mdict, const U16 type, int inmemory)
{
  LTrie Lt;
  FILE *stream;
  int eof,numtokens;
  ...
  GERLETTER conv[MAX_TOKEN_LENGTH];
  ...
  Lt = make_new_ltrie(LTRIE_CHUNK_SIZE,LTRIE_CHUNK_SIZE,type);
  //if (inmemory == 0) return Lt;
  ...
  while (eof==0)
      ...
      else
	{
	  ...
	  if (inmemory)
	  {
	  if (split_morph_phonemes(pin, &phonemehandle, &rule , Mdict) == 0) // may add to pool
	    {
	  ...
	  {
		  phonemehandle = 0;   // enter mock entry if ltrie is not used.
		  rule = 0;
	  }
	  ...
	  if (contains_umlaut(pin))
	    {
	      pin = convert_to_nonumlaut(pin,conv);
	      insert_string_to_ltrie(pin,phonemehandle,rule, &Lt);
	    }
	}
    }
