...
#if defined _UNIX_LIKE_ && !defined VXWORKS
#include <sys/time.h>
...
#include "tts.h"
#ifdef USE_PORTAUDIO
int PaCallbackFunction(void *inputBuffer,void *outputBuffer,
...
static void ClosePlayDeviceEx( HPLAY_AUDIO_T , int );
...
#if defined _UNIX_LIKE_ && !defined __osf__ && !defined USE_PORTAUDIO
  OSS_WaveInit();
#endif
...
#ifdef USE_PORTAUDIO
	uiNumberOfWaveOutDevices=Pa_CountDevices();
#else
...
#ifdef USE_PORTAUDIO
  pPlayAudio->port_audio_output_buffer=NULL;
  pPlayAudio->port_audio_samples_per_buffer=0;
  pPlayAudio->ClosePortAudioStream=0;
  pPlayAudio->FirstBuffer=0;
#endif
  ...
  if ( pPlayAudio != NULL )
      ...
      if ( pPlayAudio->bHandleValid )
	{
	  ...
	  if ( pPlayAudio->dwDeviceOptions & PA_OWN_DEVICE )
	    {
#ifdef USE_PORTAUDIO
		  Pa_CloseStream(pPlayAudio->hWaveOut);
...
#if defined __linux__ || defined VXWORKS || defined _APPLE_MAC_
...
#endif
#ifdef USE_PORTAUDIO
  DevInfo=Pa_GetDeviceInfo(pPlayAudio->uiSelectedDeviceID);
#else
  /********************************************************************/
  /*  Return an error if NULL handle.                                 */
  /********************************************************************/
...
{
#if (!defined _WIN32 && !defined _UNIX_LIKE_) || defined __osf__// NAL warning removal
  LPDWORD pdwVol;
#endif
...
#if defined _WIN32 || defined _UNIX_LIKE_ && !defined __osf__
#ifdef USE_PORTAUDIO
#else
                                   ...
                                   LPWAVEFORMATEX pWaveFormat )
{
  ...
  /********************************************************************/
#ifdef USE_PORTAUDIO
	uiDeviceID=Pa_GetDefaultOutputDeviceID();
...
#ifdef USE_PORTAUDIO
	if (pPlayAudio->ClosePortAudioStream)
	{
		if (pPlayAudio->hWaveOut)
		{
			Pa_CloseStream(pPlayAudio->hWaveOut);
			Process_MM_WOM_CLOSE_Message(pPlayAudio);
		}
                                            ...
                                            ATYPE_T aParam )
{
  ...
  {
#ifdef USE_PORTAUDIO
	  uiSelectedDeviceID=Pa_GetDefaultOutputDeviceID();
#else
...
#ifdef USE_PORTAUDIO
  pPlayAudio->dwDeviceSupport = WAVECAPS_LRVOLUME | WAVECAPS_PLAYBACKRATE | 
	  WAVECAPS_VOLUME  |WAVECAPS_SAMPLEACCURATE ;
#else
  LOCK_MME_CALL(
...
static void StartAudioPlaying( HPLAY_AUDIO_T pPlayAudio )
{
  ...
  // tek 06nov97 hold this mutex for the duration
	// MGS Why would we want to holds mutex longer than necessary???
...
#ifdef USE_PORTAUDIO
	  Pa_StartStream(pPlayAudio->hWaveOut);
	  pPlayAudio->FirstBuffer=1;
#endif
    /******************************************************************/
...
//    OP_LockMutex( pPlayAudio->hmxQueueCount );
    ...
    {
#ifdef USE_PORTAUDIO
		Pa_StopStream(pPlayAudio->hWaveOut);
#else
...
#ifdef USE_PORTAUDIO
	if (pPlayAudio->FirstBuffer==1)
	{
		uiOldWriteLength =  PORT_AUDIO_BUF_SIZE<<1;
	}
	else
	{
		uiOldWriteLength = PORT_AUDIO_BUF_SIZE<<1;
...
//      ClosePlayDeviceRequest( pPlayAudio );
  ...
  OP_UnlockMutex( pPlayAudio->hmxQueueCount );
	...
	if (uiOldWriteLength>pPlayAudioRing->iQueueCount)
	{
		uiOldWriteLength=pPlayAudioRing->iQueueCount;
		pPlayAudioRing->iQueueCount=0;
...
#ifdef USE_PORTAUDIO
		if (uiWriteSize>(PORT_AUDIO_BUF_SIZE<<1))
		{
			uiWriteSize=(PORT_AUDIO_BUF_SIZE<<1);
		}
#elif defined __QNX__
		if (uiWriteSize>(568))
		{
...
{
	ClosePlayDeviceEx(  pPlayAudio, 0);
}


static void ClosePlayDeviceEx( HPLAY_AUDIO_T pPlayAudio, int from_mm_wom_done)
{
  DWORD dwPosition;
  ...
  else
  {
	...
	{
		pPlayAudio->ClosePortAudioStream=1;
	}
#else
    LOCK_MME_CALL( waveOutClose( pPlayAudio->hWaveOut ))
#endif
...
#ifdef USE_PORTAUDIO
  memcpy(pPlayAudio->port_audio_output_buffer,pPlayAudioRing->pQueueOutput,uiLength);
  if (uiLength<(PORT_AUDIO_BUF_SIZE<<1))
  {
	  memset(pPlayAudio->port_audio_output_buffer+uiLength,0,
		  (PORT_AUDIO_BUF_SIZE<<1)-uiLength);
  }
  mmStatus=0;
#else
...
#ifdef USE_PORTAUDIO
  Process_MM_WOM_OPEN_Message(pPlayAudio,0);
#endif
...
#ifdef USE_PORTAUDIO
	if (dwBytesPerSample==1)
	{
		PaFormat=paInt8;
	}
	else if (dwBytesPerSample==2)
	{
		PaFormat=paInt16;
	}
#endif
...
#endif
#ifdef USE_PORTAUDIO
    mmStatus=Pa_OpenStream((PortAudioStream **)&(pPlayAudio->hWaveOut),
		paNoDevice,
		0,
		0,
		NULL,
		...
		NULL,
		(double)dwSampleRate,
		PORT_AUDIO_BUF_SIZE,//568 framesperbuffer
		PORT_AUDIO_NUM_BUFS,//8 numberofbuffers
		0,
		PaCallbackFunction,
		pPlayAudio);
...
//	Sleep(100);
	...
	//error codes have to be converted here.... HELPME
...
#ifdef USE_PORTAUDIO
	  mmStatus=Pa_AbortStream(pPlayAudio->hWaveOut);
	  mmStatus=Pa_StopStream(pPlayAudio->hWaveOut);
	  pPlayAudio->dwAudioPlayState = AUDIO_STATE_PLAY;
...
#ifdef USE_PORTAUDIO
  pMmt->u.sample=Pa_StreamTime(pPlayAudio->hWaveOut);
  pMmt->u.cb=pMmt->u.sample<<1;
#else
...
{
	int ret;
	HPLAY_AUDIO_T pPlayAudio=((HPLAY_AUDIO_T)userdata);

	if (    pPlayAudio->dwAudioDeviceState == AUDIO_DEVICE_STARTING_UP)
	{
		OP_LockMutex( pPlayAudio->hmxAudioDeviceState );
		pPlayAudio->dwAudioDeviceState = AUDIO_DEVICE_ACTIVE;
		OP_UnlockMutex( pPlayAudio->hmxAudioDeviceState );
	...
	memset(outputBuffer,0,framesperbuffer<<1);

	if (pPlayAudio->dwAudioPlayState == AUDIO_STATE_RESET)
	{
		return(1);
	}

	pPlayAudio->port_audio_output_buffer=(char *)outputBuffer;
	pPlayAudio->port_audio_samples_per_buffer=framesperbuffer;
	
	ret=Process_MM_WOM_DONE_Message(pPlayAudio,0);
	if (ret)
	{
		ClosePlayDeviceEx(pPlayAudio,1);
	}
	pPlayAudio->FirstBuffer=0;
	return(ret);
}
