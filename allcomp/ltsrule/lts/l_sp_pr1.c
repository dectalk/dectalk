...
//		lbuf[1].l_ip = NULL;
		...
		{
			ls_util_send_phone(phTTS, SP_A);         /* Eh?                  */
		}
		ls_util_send_phone(phTTS, WBOUND);
	}
}
 ...
 *
 *      Arguments: LPTTS_HANDLE_T phTTS	Texto-to-speech handle
 *				   LETTER * lp
 *
 *      Return Value: void
 *
		...
		else if (lp->l_ch=='1')
		{
			if ((lp+1)->l_ch=='0' && (lp+2)->l_ch=='0')
			{
				if (pLts_t->ord == 1)
				{
					...
					ls_util_send_phone(phTTS, SP_O);
					pLts_t->ord = 0;
				}
				else if (pLts_t->ord == 2)
				{
					ls_util_send_phone_list(phTTS, pordonehundred);
					ls_util_send_phone(phTTS, SP_A);
					pLts_t->ord = 0;
				}
				else
				{
...
int ls_proc_is_date(LETTER *llp, LETTER *rlp)
{
	...
	if (++llp==rlp || !IS_ALPHA(llp->l_ch))
		return (FALSE);
	buf[2] = llp->l_ch;
	// CAB	Removed warnings by typecast of months 
	for (mp = (unsigned char **)&months[0]; (cp = *mp++) != (unsigned char *)NULL;) /* Validate.    	*/
	{
...
void ls_proc_do_frac(LPTTS_HANDLE_T phTTS, LETTER * llp, LETTER * rlp)
{
	...
	*/
	...
	{
		ls_util_send_phone(phTTS, SP_U);
		ls_util_send_phone(phTTS, SP_N);
	...
	else 
	{
		...
		/* GL 11/05/1996, use SP_S to mark plural. have no idea what "ud" do */
		/* also comment out the sending of "ppercent" */
		if (pflag != FALSE) ls_util_send_phone(phTTS, SP_S);
	}	 
}