 ...
 * 008	DR/GL	07/21/1997		Created, copied from l_us_pr1.c
 * 009  GL      10/19/1997      set off numbers with a glotal stop(request from EAB)
 * 010  GL      10/31/1997      modify code for UK
 * 011  EAB     11/21/1997      EAB BATS 266  remove "19xx" expansion 
 * 012	MGS		01/08/1997		BATS#412 and #548  fixed ½ and ¼ problem
 * 013	MGS		04/13/2000		Changes for integrated phoneme set 
 * 014	NAL		05/24/2000		Fuction def change, warning removal
 * 015 	CAB		10/18/2000		Changed copyright info
 * 016	MGS		01/08/2000		Removed l_ip from LETTER structure
 * 017	CAB		04/26/2002		Removed warnings by typecast
 */
#include "ls_def.h"
 ...
 *
 *      Arguments:	LPTTS_HANDLE_T phTTS	Text-to-speech
 *					LETTER *lp
 *
 *      Return Value: void
 *
...
void ls_proc_do_digit_group(LPTTS_HANDLE_T phTTS, unsigned char buf[3], int oflag)
{
	if (buf[0] != '0') 
	{
		...
		if (buf[1]=='0' && buf[2]=='0') 
		{
			if (oflag != FALSE)
			{
				ls_util_send_phone(phTTS,UK_TH);
			}
			return;
		}
...
/* eab 10/97 set off numbers with a glotal stop*/
	ls_util_send_phone(phTTS,UK_Q);
	...
	else 
	{
		...
		{												/* Make plural.			*/
			...
			if (tlp2>tlp1+1 && (tlp2-2)->l_ch=='1')
				ud = '0';
			ls_util_send_phone(phTTS,ud=='2'||ud=='3' ? UK_Z : UK_S);
		}
	}
	if (tlp2 != rlp) 
	{													/* Must be "%".			*/
 ...
 *
 *	Arguments:
 *		LPTTS_HANDLE	Text-to-speech handle
 *		LETTER *llp		The left bounding pointer to the word
 *		LETTER *rlp     The right bounding pointer to the word
 *
 *	Return Value:
