...
/* GL 09/24/1997  add LDS_BUILD flag for LDS run */
	...
	if (DT_DBG(LTS_DBG,0x040))
	{
		...
		fprintf((FILE *)pKsd_t->dbglog,"FC(%d)(%08x)\n",pLts_t->cur_word_index,pLts_t->word_info[pLts_t->cur_word_index].form_class);
#endif
		printf("FC(%d)(%08x)\n",pLts_t->cur_word_index,pLts_t->word_info[pLts_t->cur_word_index].form_class);
#else
#ifndef ARM7_NOSWI
...
#endif
#ifdef ITALIAN
	linp = pKsd_t->lang_lts[LANG_italian];
#endif
#ifdef FRENCH
...
#endif
#ifdef ITALIAN    /* REWT BACHUS include August 19, 2004 */
  pLts_t->it_bachus_wordgrammarinfo = &IT_Bach_WordGrammar;
//  pLts_t->bachus_trennung = &HyphenTOK;     /* REWT: Mar 27 2003 */
#endif
}
...
#endif	
#ifdef ITALIAN
				if ( ((sIPhone>>PSFONT)&0xFF) == PFIT || (sIPhone>=100 && sIPhone<=PHO_SYM_TOT))
#endif					
#ifdef FRENCH
...
#ifdef ITALIAN
        kinp = pKsd_t->lang_ph[LANG_italian];
#endif
...
//#endif
			...
			if (((*phone & 0xff) >= 111) && ((*phone & 0xff) <= 118))
			   printf("\n"); 
