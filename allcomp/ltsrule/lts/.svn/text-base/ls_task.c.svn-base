 ...
 * 110	MGS		07/27/2004	Fixed phone numbers dropping the last digit
 * 111	MGS		11/22/2004	Fix for CNN in spanish, merging error
 *
 */
...
#endif
			pLts_t->first_pass=1;
			temp_pronflag=pKsd_t->pronflag;
			pLts_t->input_array[pLts_t->cur_input_pos]=0;

			if (((pCmd_t->skip_mode & SKIP_parser)==0) &&
				((pKsd_t->modeflag & MODE_READING)==0))
				ls_task_parse_sentence(phTTS);

			pLts_t->input_array[pLts_t->cur_input_pos]=0;
			pKsd_t->pronflag=temp_pronflag;
...
void lts_main_loop(LPTTS_HANDLE_T phTTS)
{
...
#endif
	{
		...
		if (pLts_t->cur_read_pos>=pLts_t->cur_input_pos)
			second_time=1;
		if (pLts_t->stuff_left_in_cword)
			pLts_t->stuff_left_in_cword=0;
#endif
...
int ls_task_fix_german_vowels(PLTS_T pLts_t, LETTER *llp, LETTER **rlp)
{
	...
	for (elp=llp;elp->l_ch!=EOS;elp++)
	{
		...
		if (ls_lower[((elp+1)->l_ch)] == 'e')
		{
			...
			{
			case 'a':
				elp->l_ch=(unsigned char)0xE4;	//'ä';
				move=1;
				break;
			case 'A':
				elp->l_ch=(unsigned char)0xC4;	//'Ä';
				move=1;
				break;
			case 'o':
				elp->l_ch=(unsigned char)0xF6;	//'ö';
				move=1;
				break;
			case 'O':
				elp->l_ch=(unsigned char)0xD6;	//'Ö';
				move=1;
				break;
			case 'u':
				elp->l_ch=(unsigned char)0xFC;	//'ü';
				move=1;
				break;
			case 'U':
				elp->l_ch=(unsigned char)0xDC;	//'Ü';
				move=1;
				break;
			}
			if (move)
...
//				ls_util_send_phone(phTTS,S1);
				...
				/* GL 11/16/1998, BATS#721 fix number with currency sign */
				ls_util_send_phone(phTTS,WBOUND);
				ls_util_send_phone(phTTS,IT_D);
//				ls_util_send_phone(phTTS,S1);
				ls_util_send_phone(phTTS,IT_E);
#endif
				if (type==0x80)
				{
...
/* GL 02/14/1997, add pural for Spanish */
	...
	if ( llp!=rlp-1 && ((rlp-1)->l_ch=='F' || (rlp-1)->l_ch=='f' || (rlp-1)->l_ch==0x80)) { /* 20F, 12.23F */	//0x80='€'
											 /*
											 8-1 : CURRENCY SYMBOL.
...
#else
		if (lp2+3 == rlp && lp2->l_ch==(unsigned char)0xe8 && (lp2+1)->l_ch=='m' 	//0xe8='è'
			&& ((lp2+2)->l_ch=='e')) 
			{
...
//			Fini=2;
		...
		&& (llp+1)->l_ch==(unsigned char)0xe8 && (llp+2)->l_ch=='r'	//0xe8='è'
		&& (llp+3)->l_ch=='e' ) 
		{
...
//			Fini=2;
    ...
    /* there is 1 character after last digit like 10% or 10° */
		if ( (c=lp2->l_ch)=='%' || c==(unsigned char)0xb0) 	//0xb0='°'
		{
			ls_proc_do_sign (phTTS, pLts_t->sign);
//			sendindex (llp, lp1);
...
{	
	...
	{{"was" }, { US_W,  S2, US_AH, US_Z,   SIL }, 0x00820020},
...
/* GL 11/03/1997  add UK code */
	...
	{{"was" }, { UK_W,  S2, UK_AH, UK_Z,   SIL }, 0x00820020},
...
#include "proverbs_it.h"
#endif
#endif

#if defined ARM7 || defined _UNIX_LIKE_
...
{
#ifndef ARM7
#if defined ENGLISH || defined ITALIAN
	PLTS_T  pLts_t;
...
void ls_task_parse_sentence(LPTTS_HANDLE_T phTTS)
{
	...
	for (i=0;i<pLts_t->cur_input_pos;i++)
	{
		...
		stat=ls_dict_blook(phTTS,&cur_word[start],&cur_word[k],0);

		if (stat==0 && cur_word[k-1].l_ch=='.')
		{
			stat=ls_dict_blook(phTTS,&cur_word[start],&cur_word[k-1],FABBREV);
		}
