 ...
 *							array.
 * 0008 6/94 EAB		Added time_value_specfied for merge of ph88
 * 0009 15-DEC-94 cjl	Added pre-processor code to prevent dupilcate
 *							inclusions.
 */
...
#define	NSAMP_FRAME	64		/* # of samples per output frame.	*/
#define	SPDEF_PARS	22		/* # of words in a speaker definition.	*/
#ifdef MINIMAL_SYNTH
#define	NPHON_MAX	50	/* Size of phone[] and struc[] arrays.	*/
#else
...
#ifdef MINIMAL_SYNTH
#define SAFETY	2	/* Offset between shared arrays such as phonemes[SAFETY]
			 * and allophons[0] */
#else
...
#ifdef MINIMAL_SYNTH
#define	GUARD	-10					/* WBOUND => COMMA guardband	*/
//eab -10 becuase symbol array is larger because we subtract com phons
#else
...
long klrmul(long,long);
long kllmul(long, long *);
long klrdiv(long,long);
long klldiv(long,long *);
short muldv();
short mulsh1();
short mulsh2();
short mulsh3();
