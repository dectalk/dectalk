...
int par_match_standard(unsigned char *current_rule,
					   ...
					   int lookahead,
					   int break_on_min_match);
__inline void par_copy_return_value(preturn_value_t dest,
						   preturn_value_t src);
...
__inline void par_copy_index_list(pindex_data_t dest_index,
						 ...
						 short		   src_pos,
						 short		   length);
__inline short par_is_index_set(pindex_data_t indexes,
					   short pos);
__inline void par_set_return_level(short *return_rule,
						  short *return_level,
						  short go_rule);
__inline short par_get_return_level(short *return_rule,
						   short *return_level,
...
int main(void)
{
	...
	{                                       			
		...
		par_process_input(pKsd_t,input_array,new_input_array,output_array,dict_hit_array,input_indexes,new_input_indexes,output_indexes,0x00000001,0x00000020,0,0,&match_array,&ret_value);
		...
		par_process_input(pKsd_t,input_array,new_input_array,output_array,dict_hit_array,input_indexes,new_input_indexes,output_indexes,0x00000001,0x00000002,1,0,&match_array,&ret_value);
#ifndef SINGLE_PRINT
#ifndef NO_PRINT
		...
		par_process_input(pKsd_t,input_array,new_input_array,output_array,dict_hit_array,input_indexes,new_input_indexes,output_indexes,0x00000001,0x00000100,2,0,&match_array,&ret_value);
#ifndef NO_PRINT
		par_print_output(output_array,output_indexes);						/* print the results of the matching */
#endif
#endif
...
__inline void par_initialize_arrays(pmatch_arrays_t match_arrays)
{                               
	memset(match_arrays,0,sizeof(struct match_arrays_s));
}
					   ...
					   prange_value_t range_value)
{
	...
	if (ret_value->optional!= -1)
	{
		...
		new=new_length+ret_value->output_pos-1;
		while (pos<off) /* go until the offset if equal to the position */
		{
			/* copy the character in the current string into its new location */
			output_array[new]=output_array[off];                                
			par_copy_index(output_indexes,new,output_indexes,off);
			...
			memcpy(output_array+new,buf,length);
...
{           
	...
	ret_value->output_offset=0;
	ret_value->value=SUCCESS;	
	par_match_rule(current_rule,DICTIONARY_STATE,input_array,temp_output,temp_indexes,temp_indexes,match_array,ret_value,'/',1);
...

{
	...
	short dict_number;
	short value=0;
	short rev_same=0,for_same=0,npos;
	short save_for;
#ifdef DEBUG
		 				   ...
		 				   preturn_value_t ret_value)
{
	...
	if (ret_value->optional!= -1)
	{
		...
		while (i<ret_value->output_offset && (has_cons==0 || has_vowel==0))
		{
		...
		if (has_cons && has_vowel && (ret_value->output_offset-ret_value->output_pos)>=2)
		{
#ifdef DEBUG
