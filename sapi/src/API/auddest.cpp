...
// 26jul01	cab		Commented out unused variables
//
...
#ifdef FIX_THE_NOTIFICATIONS
	if (!m_fClaimed || !m_fStarted)
	{
		OpenAudioDevice();
		Sleep(1);
	}
...
HRESULT CMode::OpenAudioDevice()
{
	...
	// Claim the device		  
	if (!m_fClaimed && !m_fPaused)// && pAudioParams->pAudioQueue->iQueueCount!=0)
	{
#ifdef SAPI_GROUP_H_TIMING
		QWORD qwCount=0;
		...
		// Start the audio so the notifications come out at the right times
		hRes = m_pAudioIAudio->Start();
		if (hRes)
		  return hRes;
		m_fStarted = TRUE;
